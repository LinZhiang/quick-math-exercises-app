...
/**
 * 入库时若把结束围栏写成了 ```js，会把后文粘进代码块。
 * 围栏内再遇到 ```lang 时：先闭合，再视下一行决定要不要新开一块。
 */
export function repairFusedMarkdownFences(md) {
...
/** 整理一个代码块正文。 */
export function tidyJsFenceBody(code, { expand = true } = {}) {
  let s = String(code ?? '')
    .replace(/\r\n/g, '\n')
    .replace(/\t/g, '  ')
  if (expand) {
...
export function tidyJsFencesInMarkdown(md) {
  const repaired = repairFusedMarkdownFences(md)
  FENCE_RE.lastIndex = 0
  return repaired.replace(FENCE_RE, (_all, lang, body) => {
