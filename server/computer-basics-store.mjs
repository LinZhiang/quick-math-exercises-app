...
const CATALOG_BACKUP_DIR = path.join(__dirname, 'data', 'handout-catalog-backups', 'computer-basics')
...
function seedFromFrontendJson() {
  if (!fs.existsSync(SEED_FILE)) {
    throw new Error(
      '计算机基础尚未入库，且找不到种子文件 src/data/computer-overview.json。请先放入讲义数据。',
    )
  }
...
export function readComputerBasicsCatalog() {
...
export function resolveComputerBasicsMedia(fileName) {
  ensureComputerBasicsStore()
...
function rotateCatalogBackup(prevRaw) {
  try {
    ...
    console.warn('[computer-basics] 目录备份失败：', e instanceof Error ? e.message : e)
  }
}
...
/**
 * 从题库包导入「计算机专业知识」整棵树（第一部分 + 第二部分 WIN10/Excel）。
 * 已存在的讲义默认跳过，避免重复拆图。
 */
export function importComputerBasicsFromBankPack(packPath, { skipExisting = true } = {}) {
  ...
  // eslint-disable-next-line no-console
  console.log(
    `[computer-basics] 已导入计算机专业知识：新增 ${wrote} 条，跳过 ${skipped} 条，拆出约 ${imageCount} 张插图`,
  )
...
export async function pullComputerBasicsFromCloud() {
  ...
  const pack = await fetchCloudHandoutPack({
    apiPrefix: '/api/computer-basics',
    mediaPrefix: '/api/media/computer-basics',
  })
...
export function attachComputerBasicsRoutes(app) {
  ...
  app.post('/api/computer-basics/pull-cloud', requireAdmin, async (req, res) => {
    try {
      const result = await pullComputerBasicsFromCloud()
  ...
  app.get('/api/computer-basics/tree', (req, res) => {
    try {
      ...
      res.status(500).json({
        ok: false,
        message: e instanceof Error ? e.message : '读取计算机基础目录失败',
      })
    }
  })
  ...
  app.get('/api/media/computer-basics/:file', (req, res) => {
    try {
      const media = resolveComputerBasicsMedia(req.params.file)
      if (!media) {
  ...
  app.post('/api/computer-basics/nodes', requireAdmin, (req, res) => {
    try {
  ...
  app.delete('/api/computer-basics/nodes/:id', requireAdmin, (req, res) => {
    try {
      const catalog = readRawCatalog()
      const hit = findNode(catalog.tree, String(req.params.id))
      if (!hit) {
  ...
  app.post('/api/computer-basics/items', requireAdmin, (req, res) => {
    try {
  ...
  app.patch('/api/computer-basics/items/:id', requireAdmin, (req, res) => {
    try {
      const id = String(req.params.id)
      const current = readComputerBasicsItem(id)
      if (!current) {
        res.status(404).json({ ok: false, message: '未找到该讲义' })
        return
      }
  ...
  app.delete('/api/computer-basics/items/:id', requireAdmin, (req, res) => {
    try {
      const id = String(req.params.id)
      const catalog = readRawCatalog()
      const hit = findEntry(catalog.tree, id)
