...
async function reloadKeepExpand(opts?: {
  ...
  const next = await loadComputerBasicsTree(false)
...
async function onTogglePrivate(row: ComputerTreeRow) {
  const next = !row.ownPrivate
  try {
    await withBusy(next ? '正在设为私密…' : '正在设为公开…', async () => {
      if (row.kind === 'branch') await setComputerNodePrivate(row.id, next)
      else await setComputerItemPrivate(row.id, next)
