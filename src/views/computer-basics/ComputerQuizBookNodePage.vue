...
const selectedNode = computed(() => findComputerQuizBookNode(bookTree.value, nodeId.value))
...
const kindOptions = computed(() => {
  ...
  const order: ComputerQuizKind[] = ['choice', 'judge', 'calc', 'short']
...
const preparedQuestions = computed((): ComputerQuizQuestion[] =>
...
const quizItem = computed((): ComputerHandoutItem => {
...
function computerQuizPlainText(s: string) {
  return String(s ?? '')
    .replace(/<[^>]+>/g, ' ')
    .replace(/[`*_#]/g, '')
    .replace(/\s+/g, ' ')
...
function rowListTitle(row: StoredComputerQuizRecord) {
  const term = computerQuizPlainText(row.term)
  if (term) return term
  const stem = computerQuizPlainText(row.stem)
...
function rowFavorited(fp: string) {
  return isComputerQuizFavorite(fp)
}

function onToggleFavorite(row: StoredComputerQuizRecord) {
  const r = toggleComputerQuizFavorite({
    fingerprint: row.fingerprint,
    kind: row.kind,
    term: row.term,
    stem: row.stem,
      ...
      <template v-else>
        ...
        <template v-else-if="detailRow">
          ...
          <div ref="detailBodyRef" class="cb-book__detail is-page">
            <p class="cb-book__kind">{{ computerQuizKindLabel(detailRow.kind) }}</p>
            <p class="cb-book__from">{{ rowPath(detailRow) }}</p>
            <RichTextView :html="displayOf(detailRow).stem" />
