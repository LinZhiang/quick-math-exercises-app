...
const test = useChineseWordMemorizationTest()
...
defineExpose({
  ...
  startWith(questions: WordMemorizationQuestion[], keyReview?: ChineseKeyReviewMeta) {
    test.resetToIdle()
    test.startQuiz(questions, keyReview ? { keyReview } : undefined)
  },
})

function buildAssistPrompt(q: WordMemorizationQuestion): string {
  ...
  const correct =
    q.questionType === 'meaning-to-word' ? q.term : q.options[q.correctIndex] ?? ''
  return [
    `题型：${wordMemorizationQuestionTypeLabel(q.questionType)}`,
    `目标词语：${q.term}`,
    `题干：${q.stem}`,
    `选项：\n${opts}`,
    `学员选择：${chosen}`,
    ...
    '请讲解本题涉及的词语辨析（非成语）、近义区别与记忆方法，并结合选词填空常见考法。',
  ]
    .filter(Boolean)
    .join('\n\n')
}
...
async function runAssistExplain() {
  ...
  try {
    await startAssist({
      initialUser: userMsg,
      displayUser: '请讲解本题词语辨析',
      system: WORD_MEM_ASSIST_SYSTEM,
      fetch: () =>
...
watch(
  ...
  (fp) => {
    favorited.value = fp ? isChineseWordMemorizationFavorite(fp) : false
  },
)
...
async function onToggleFavorite() {
  const q = test.currentQuestion
  if (!q) return
  const r = toggleChineseWordMemorizationFavorite(q)
  ...
  <div class="chinese-idiom-panel">
    <template v-if="test.phase === 'idle' || test.phase === 'loading'">
      <p class="mode-section__hint">
        针对公考/事业编言语理解高频非成语词语（实词、虚词、关联词等），不含四字成语；混合选释义/选词语。
        AI 每轮随机混合「选释义」「选词语」题型，每轮 {{ test.questionCount }} 题，四选一。
        正计时，提交后暂停并公布答案，点「下一题」继续。
      </p>
      <p class="practice-completion-line"><PracticeCompletionStat mode-id="chinese-word-memorization" /></p>
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        >复习题</span>
        <span v-if="test.currentQuestion">{{ wordMemorizationQuestionTypeLabel(test.currentQuestion.questionType) }}</span>
      ...
      <div v-if="test.currentQuestion" class="chinese-quiz__stem">
        <p
          v-if="shouldShowWordMemorizationTermBeforeSubmit(test.currentQuestion)"
          class="chinese-quiz__term"
        >
          {{ test.currentQuestion.term }}
        </p>
        <p class="chinese-quiz__question">{{ buildWordMemorizationDisplayStem(test.currentQuestion) }}</p>
      </div>
        ...
        >
          {{
            test.results[test.results.length - 1]?.correct
              ? '回答正确'
              : `回答错误 · 正确答案：${
                  test.currentQuestion.questionType === 'meaning-to-word'
                    ? test.currentQuestion.term
