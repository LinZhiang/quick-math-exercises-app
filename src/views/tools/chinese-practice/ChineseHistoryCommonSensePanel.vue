...
defineExpose({
  ...
  startWith(questions: HistoryCommonSenseQuestion[], keyReview?: ChineseKeyReviewMeta) {
    test.resetToIdle()
    test.startQuiz(questions, keyReview ? { keyReview } : undefined)
  },
})

function buildAssistPrompt(q: HistoryCommonSenseQuestion): string {
  ...
  return [
    `题型：${historyCommonSenseQuestionTypeLabel(q.questionType)}`,
    `知识点：${q.term}`,
    `题干：${q.stem}`,
    `选项：\n${opts}`,
    `学员选择：${chosen}`,
...
async function runAssistExplain() {
  ...
  try {
    await startAssist({
      initialUser: userMsg,
      displayUser: '请讲解本题历史常识',
      system: HIST_ASSIST_SYSTEM,
      fetch: () =>
...
watch(
  ...
  (fp) => {
    favorited.value = fp ? isChineseHistoryCommonSenseFavorite(fp) : false
  },
)
...
async function onToggleFavorite() {
  const q = test.currentQuestion
  if (!q) return
  const r = toggleChineseHistoryCommonSenseFavorite(q)
...
<template>
  <div class="chinese-idiom-panel">
    <template v-if="test.phase === 'idle' || test.phase === 'loading'">
      ...
      <p class="practice-completion-line"><PracticeCompletionStat mode-id="chinese-history-common-sense" /></p>
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        >复习题</span>
        <span v-if="test.currentQuestion">{{ historyCommonSenseQuestionTypeLabel(test.currentQuestion.questionType) }}</span>
        ...
        <template v-else>
          ...
          <div v-if="assistHistory.length" class="chinese-quiz__assist-followup">
            <el-input
              ...
              type="textarea"
              :rows="2"
              placeholder="继续追问，例如：和「洋务运动」怎么区分？"
