...
const test = useChineseRhetoricUsageTest()
...
defineExpose({
  ...
  startWith(questions: RhetoricUsageQuestion[], keyReview?: ChineseKeyReviewMeta) {
    test.resetToIdle()
    test.startQuiz(questions, keyReview ? { keyReview } : undefined)
  },
})

function buildAssistPrompt(q: RhetoricUsageQuestion): string {
  ...
  return [
    `题型：${rhetoricUsageQuestionTypeLabel(q.questionType)}`,
    `知识点：${q.term}`,
    `题干：${q.stem}`,
    `选项：\n${opts}`,
    `学员选择：${chosen}`,
...
async function runAssistExplain() {
  ...
  try {
    await startAssist({
      initialUser: userMsg,
      displayUser: '请讲解本题修辞运用',
      system: RHETORIC_ASSIST_SYSTEM,
      fetch: () =>
...
watch(
  ...
  (fp) => {
    favorited.value = fp ? isChineseRhetoricUsageFavorite(fp) : false
  },
)
...
async function onToggleFavorite() {
  const q = test.currentQuestion
  if (!q) return
  const r = toggleChineseRhetoricUsageFavorite(q)
  ...
  <div class="chinese-idiom-panel">
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        >复习题</span>
        <span v-if="test.currentQuestion">{{ rhetoricUsageQuestionTypeLabel(test.currentQuestion.questionType) }}</span>
