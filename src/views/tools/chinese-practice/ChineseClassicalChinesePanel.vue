...
const test = useChineseClassicalChineseTest()
...
defineExpose({
  ...
  startWith(questions: ClassicalChineseQuestion[], keyReview?: ChineseKeyReviewMeta) {
    test.resetToIdle()
    test.startQuiz(questions, keyReview ? { keyReview } : undefined)
  },
})

function buildAssistPrompt(q: ClassicalChineseQuestion): string {
  ...
  return [
    `题型：${classicalChineseQuestionTypeLabel(q.questionType)}`,
    `知识点：${q.term}`,
    `题干：${q.stem}`,
    `选项：\n${opts}`,
    `学员选择：${chosen}`,
...
async function runAssistExplain() {
  ...
  try {
    await startAssist({
      initialUser: userMsg,
      displayUser: '请讲解本题文言知识',
      system: CLASSICAL_ASSIST_SYSTEM,
      fetch: () =>
...
watch(
  ...
  (fp) => {
    favorited.value = fp ? isChineseClassicalChineseFavorite(fp) : false
  },
)
...
async function onToggleFavorite() {
  const q = test.currentQuestion
  if (!q) return
  const r = toggleChineseClassicalChineseFavorite(q)
  ...
  <div class="chinese-idiom-panel">
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        >复习题</span>
        <span v-if="test.currentQuestion">{{ classicalChineseQuestionTypeLabel(test.currentQuestion.questionType) }}</span>
        ...
        <template v-else>
          ...
          <div v-if="assistHistory.length" class="chinese-quiz__assist-followup">
            <el-input
              ...
              type="textarea"
              :rows="2"
              placeholder="继续追问，例如：「走」的古今义还有哪些？"
