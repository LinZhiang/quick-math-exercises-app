...
defineExpose({
  ...
  startWith(questions: IdiomRecognitionQuestion[], keyReview?: ChineseKeyReviewMeta) {
    test.resetToIdle()
    test.startQuiz(questions, keyReview ? { keyReview } : undefined)
  },
})

function buildAssistPrompt(q: IdiomRecognitionQuestion): string {
  ...
  return [
    `题型：${idiomQuestionTypeLabel(q.questionType)}`,
    `目标词语：${q.term}`,
    `题干：${q.stem}`,
    `选项：\n${opts}`,
    `学员选择：${chosen}`,
...
watch(
  ...
  (fp) => {
    favorited.value = fp ? isChineseFavorite(fp) : false
  },
)
...
async function onToggleFavorite() {
  const q = test.currentQuestion
  if (!q) return
  const r = toggleChineseFavorite(q)
  ...
  <div class="chinese-idiom-panel">
    ...
    <template v-else-if="test.phase === 'running'">
      ...
      <div v-if="test.currentQuestion" class="chinese-quiz__stem">
        <p
          v-if="shouldShowIdiomTermBeforeSubmit(test.currentQuestion)"
          class="chinese-quiz__term"
        >
          {{ test.currentQuestion.term }}
        </p>
        <p class="chinese-quiz__question">{{ buildIdiomDisplayStem(test.currentQuestion) }}</p>
      </div>
        ...
        <template v-else>
          ...
          <div v-if="assistHistory.length" class="chinese-quiz__assist-followup">
            <el-input
              ...
              type="textarea"
              :rows="2"
              placeholder="继续追问，例如：和「出类拔萃」怎么区分？"
