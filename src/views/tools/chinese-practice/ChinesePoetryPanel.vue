...
function buildAssistPrompt(q: PoetryRecognitionQuestion): string {
  ...
  return [
    `题型：${poetryQuestionTypeLabel(q.questionType)}`,
    `篇目：${q.term}`,
    `诗句/题干：${q.stem}`,
    `选项：\n${opts}`,
    `学员选择：${chosen}`,
...
async function runAssistExplain() {
  ...
  try {
    await startAssist({
      initialUser: userMsg,
      displayUser: '请讲解本题诗词常识',
      system: POETRY_ASSIST_SYSTEM,
      fetch: () =>
...
watch(
  ...
  (fp) => {
    favorited.value = fp ? isChinesePoetryFavorite(fp) : false
  },
)
...
async function onToggleFavorite() {
  const q = test.currentQuestion
  if (!q) return
  const r = toggleChinesePoetryFavorite(q)
  ...
  <div class="chinese-idiom-panel">
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span v-if="test.currentQuestion">{{ poetryQuestionTypeLabel(test.currentQuestion.questionType) }}</span>
      ...
      <div v-if="test.currentQuestion" class="chinese-quiz__stem">
        <p
          v-if="shouldShowPoetryTermBeforeSubmit(test.currentQuestion)"
