...
import {
  ...
  type StrengthenReasonResultRow,
} from '@/composables/logic/useStrengthenReasonTest'
import {
  STRENGTHEN_REASON_MODES,
...
const selectedDifficulty = ref<StrengthenReasonDifficulty | null>(null)
const test = reactive(useStrengthenReasonTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<StrengthenReasonResultRow | null>(null)
...
function selectDifficulty(d: StrengthenReasonDifficulty) {
...
function openResultDetail(row: StrengthenReasonResultRow) {
  detailRow.value = row
...
function optionClass(row: StrengthenReasonResultRow, idx: number) {
...
function passageHtml(q: StrengthenReasonQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: StrengthenReasonQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: StrengthenReasonQuestion | null | undefined) {
  ...
  >
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「加强论证」：找出使结论成立的前提，或最能加强论证的一项（排除他因、例证支持等）。由网页 AI
        大模型出题（需已配置），每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in STRENGTHEN_REASON_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`logic-reason-strengthen-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ strengthenReasonTopicLabel() }} ·
        {{ strengthenReasonDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>
          {{ strengthenReasonTopicLabel() }} ·
          {{ strengthenReasonDifficultyLabel(selectedDifficulty!) }}
        </span>
