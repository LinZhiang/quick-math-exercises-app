...
import {
  ...
  type DailyConclusionResultRow,
} from '@/composables/logic/useDailyConclusionTest'
import {
  DAILY_CONCLUSION_MODES,
...
const selectedDifficulty = ref<DailyConclusionDifficulty | null>(null)
const test = reactive(useDailyConclusionTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<DailyConclusionResultRow | null>(null)
...
function openResultDetail(row: DailyConclusionResultRow) {
  detailRow.value = row
...
function optionClass(row: DailyConclusionResultRow, idx: number) {
...
function passageHtml(q: DailyConclusionQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: DailyConclusionQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: DailyConclusionQuestion | null | undefined) {
  ...
  >
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「日常结论」：根据日常/科普材料推出必然成立的结论，排除过度推断。由网页 AI
        大模型出题（需已配置），每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in DAILY_CONCLUSION_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`logic-reason-daily-conclusion-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ dailyConclusionTopicLabel() }} ·
        {{ dailyConclusionDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>
          {{ dailyConclusionTopicLabel() }} ·
          {{ dailyConclusionDifficultyLabel(selectedDifficulty!) }}
        </span>
