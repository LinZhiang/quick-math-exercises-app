...
import {
  useWeakenReasonTest,
  type WeakenReasonResultRow,
} from '@/composables/logic/useWeakenReasonTest'
import {
  WEAKEN_REASON_MODES,
...
const selectedDifficulty = ref<WeakenReasonDifficulty | null>(null)
const test = reactive(useWeakenReasonTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<WeakenReasonResultRow | null>(null)
...
function selectDifficulty(d: WeakenReasonDifficulty) {
...
function openResultDetail(row: WeakenReasonResultRow) {
  detailRow.value = row
...
function optionClass(row: WeakenReasonResultRow, idx: number) {
...
function passageHtml(q: WeakenReasonQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: WeakenReasonQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: WeakenReasonQuestion | null | undefined) {
  ...
  >
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「削弱论证」：找出最能削弱结论的一项（另有他因、切断外推、质疑统计等）。由网页 AI
        大模型出题（需已配置），每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in WEAKEN_REASON_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`logic-reason-weaken-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ weakenReasonTopicLabel() }} ·
        {{ weakenReasonDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>
          {{ weakenReasonTopicLabel() }} ·
          {{ weakenReasonDifficultyLabel(selectedDifficulty!) }}
        </span>
