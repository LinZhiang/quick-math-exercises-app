...
import {
  useEvalReasonTest,
  type EvalReasonResultRow,
} from '@/composables/logic/useEvalReasonTest'
import {
  EVAL_REASON_MODES,
...
const selectedDifficulty = ref<EvalReasonDifficulty | null>(null)
const test = reactive(useEvalReasonTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<EvalReasonResultRow | null>(null)
...
function selectDifficulty(d: EvalReasonDifficulty) {
...
function openResultDetail(row: EvalReasonResultRow) {
  detailRow.value = row
...
function optionClass(row: EvalReasonResultRow, idx: number) {
...
function passageHtml(q: EvalReasonQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: EvalReasonQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: EvalReasonQuestion | null | undefined) {
  ...
  >
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「评价推理」：指出论证漏洞或正确评价某方观点（样本、人身攻击、绝对化等）。由网页 AI
        大模型出题（需已配置），每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in EVAL_REASON_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`logic-reason-eval-reason-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ evalReasonTopicLabel() }} ·
        {{ evalReasonDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>
          {{ evalReasonTopicLabel() }} ·
          {{ evalReasonDifficultyLabel(selectedDifficulty!) }}
        </span>
