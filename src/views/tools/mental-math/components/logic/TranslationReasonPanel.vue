...
import {
  ...
  type TranslationReasonResultRow,
} from '@/composables/logic/useTranslationReasonTest'
import {
  TRANSLATION_REASON_MODES,
...
const selectedDifficulty = ref<TranslationReasonDifficulty | null>(null)
const test = reactive(useTranslationReasonTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<TranslationReasonResultRow | null>(null)
...
function selectDifficulty(d: TranslationReasonDifficulty) {
...
function openResultDetail(row: TranslationReasonResultRow) {
  detailRow.value = row
...
function optionClass(row: TranslationReasonResultRow, idx: number) {
...
function passageHtml(q: TranslationReasonQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: TranslationReasonQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: TranslationReasonQuestion | null | undefined) {
  ...
  >
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「翻译推理」：把「若…则…」「只有…才…」等假言命题译成逻辑关系，再判断可推出的结论。由网页 AI
        大模型出题（需已配置），每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in TRANSLATION_REASON_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`logic-reason-translation-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ translationReasonTopicLabel() }} ·
        {{ translationReasonDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>
          {{ translationReasonTopicLabel() }} ·
          {{ translationReasonDifficultyLabel(selectedDifficulty!) }}
        </span>
