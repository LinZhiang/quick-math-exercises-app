...
import {
  ...
  type ExplainPhenomResultRow,
} from '@/composables/logic/useExplainPhenomTest'
import {
  EXPLAIN_PHENOM_MODES,
...
const selectedDifficulty = ref<ExplainPhenomDifficulty | null>(null)
const test = reactive(useExplainPhenomTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<ExplainPhenomResultRow | null>(null)
...
function selectDifficulty(d: ExplainPhenomDifficulty) {
...
function openResultDetail(row: ExplainPhenomResultRow) {
  detailRow.value = row
...
function optionClass(row: ExplainPhenomResultRow, idx: number) {
...
function passageHtml(q: ExplainPhenomQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: ExplainPhenomQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: ExplainPhenomQuestion | null | undefined) {
  ...
  >
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「解释现象」：找出最能解释某现象或化解矛盾的一项（含组合选肢）。由网页 AI
        大模型出题（需已配置），每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in EXPLAIN_PHENOM_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`logic-reason-explain-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ explainPhenomTopicLabel() }} ·
        {{ explainPhenomDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>
          {{ explainPhenomTopicLabel() }} ·
          {{ explainPhenomDifficultyLabel(selectedDifficulty!) }}
        </span>
