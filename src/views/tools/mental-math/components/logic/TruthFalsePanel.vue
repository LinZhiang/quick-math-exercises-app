...
import {
  useTruthFalseTest,
  type TruthFalseResultRow,
} from '@/composables/logic/useTruthFalseTest'
import {
  TRUTH_FALSE_MODES,
...
const selectedDifficulty = ref<TruthFalseDifficulty | null>(null)
const test = reactive(useTruthFalseTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<TruthFalseResultRow | null>(null)
...
function selectDifficulty(d: TruthFalseDifficulty) {
...
function openResultDetail(row: TruthFalseResultRow) {
  detailRow.value = row
...
function optionClass(row: TruthFalseResultRow, idx: number) {
...
function passageHtml(q: TruthFalseQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: TruthFalseQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: TruthFalseQuestion | null | undefined) {
  ...
  >
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「真假推理」：几人说话有真有假（真假数量与人数均可变化），用矛盾、对当与假设排除推出结论。由网页 AI
        大模型出题（需已配置），每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in TRUTH_FALSE_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`logic-reason-truth-false-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ truthFalseTopicLabel() }} ·
        {{ truthFalseDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>
          {{ truthFalseTopicLabel() }} ·
          {{ truthFalseDifficultyLabel(selectedDifficulty!) }}
        </span>
