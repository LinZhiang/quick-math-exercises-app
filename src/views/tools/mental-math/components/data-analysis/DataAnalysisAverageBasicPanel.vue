...
import {
  useDataAnalysisAverageBasicTest,
  type AverageBasicResultRow,
} from '@/composables/data-analysis/useDataAnalysisAverageBasicTest'
import { isAiChatConfigured } from '@/services/deepseek'
import {
  AVERAGE_BASIC_MODES,
  averageBasicDifficultyLabel,
  averageBasicTopicLabel,
  renderAverageBasicPassageHtml,
...
const selectedDifficulty = ref<AverageBasicDifficulty | null>(null)
const test = reactive(useDataAnalysisAverageBasicTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<AverageBasicResultRow | null>(null)
...
function selectDifficulty(d: AverageBasicDifficulty) {
...
function openResultDetail(row: AverageBasicResultRow) {
  detailRow.value = row
...
function optionClass(row: AverageBasicResultRow, idx: number) {
...
function plainHtml(text: string): string {
  return renderAverageBasicPassageHtml(text, [], false, text)
}
...
function passageHtml(q: AverageBasicQuestion | null | undefined) {
  if (!q?.passage) return ''
  return plainHtml(q.passage)
}

function stemHtml(q: AverageBasicQuestion | null | undefined) {
  if (!q?.stem) return ''
  return mathHtml(q.stem)
}

function explainHtml(q: AverageBasicQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「平均数——基本公式」：平均数=总量/份数、反推总量或份数、简单算术平均。简单/复杂均为纯文字；复杂含单位换算、大数估算与多步提取，对齐教材。本模块
        <strong>仅豆包</strong>出题。每轮
        {{ test.questionCount }} 题四选一。正计时，提交后暂停并公布答案（不另做数字高亮，以解析为准）。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in AVERAGE_BASIC_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`data-analysis-average-basic-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ averageBasicTopicLabel() }} · {{ averageBasicDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ averageBasicTopicLabel() }} · {{ averageBasicDifficultyLabel(selectedDifficulty!) }}</span>
