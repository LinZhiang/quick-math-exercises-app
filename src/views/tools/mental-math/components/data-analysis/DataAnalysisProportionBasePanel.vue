...
import {
  useDataAnalysisProportionBaseTest,
  type ProportionBaseResultRow,
} from '@/composables/data-analysis/useDataAnalysisProportionBaseTest'
import { isAiChatConfigured } from '@/services/deepseek'
import {
  PROPORTION_BASE_MODES,
  proportionBaseDifficultyLabel,
  proportionBaseTopicLabel,
  renderProportionBasePassageHtml,
...
const selectedDifficulty = ref<ProportionBaseDifficulty | null>(null)
const test = reactive(useDataAnalysisProportionBaseTest(selectedDifficulty))
...
function selectDifficulty(d: ProportionBaseDifficulty) {
...
function openResultDetail(row: ProportionBaseResultRow) {
  detailRow.value = row
...
function optionClass(row: ProportionBaseResultRow, idx: number) {
...
function plainHtml(text: string): string {
  return renderProportionBasePassageHtml(text, [], false, text)
}
...
function passageHtml(q: ProportionBaseQuestion | null | undefined) {
  if (!q?.passage) return ''
  return plainHtml(q.passage)
}

function stemHtml(q: ProportionBaseQuestion | null | undefined) {
  if (!q?.stem) return ''
  return mathHtml(q.stem)
}

function explainHtml(q: ProportionBaseQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      ...
      <div class="mode-grid">
        <button
          v-for="m in PROPORTION_BASE_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`data-analysis-proportion-base-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ proportionBaseTopicLabel() }} · {{ proportionBaseDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ proportionBaseTopicLabel() }} · {{ proportionBaseDifficultyLabel(selectedDifficulty!) }}</span>
