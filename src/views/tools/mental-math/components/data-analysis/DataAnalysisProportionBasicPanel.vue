...
import {
  useDataAnalysisProportionBasicTest,
  type ProportionBasicResultRow,
} from '@/composables/data-analysis/useDataAnalysisProportionBasicTest'
import { isAiChatConfigured } from '@/services/deepseek'
import {
  PROPORTION_BASIC_MODES,
  proportionBasicDifficultyLabel,
  proportionBasicTopicLabel,
  renderProportionBasicPassageHtml,
...
import DataAnalysisProportionPieChart from '@/views/tools/mental-math/components/data-analysis/DataAnalysisProportionPieChart.vue'
...
function selectDifficulty(d: ProportionBasicDifficulty) {
...
function openResultDetail(row: ProportionBasicResultRow) {
  detailRow.value = row
...
function optionClass(row: ProportionBasicResultRow, idx: number) {
...
function plainHtml(text: string): string {
  return renderProportionBasicPassageHtml(text, [], false, text)
}
...
function passageHtml(q: ProportionBasicQuestion | null | undefined) {
  if (!q?.passage) return ''
  return plainHtml(q.passage)
}

function stemHtml(q: ProportionBasicQuestion | null | undefined) {
  if (!q?.stem) return ''
  return mathHtml(q.stem)
}

function explainHtml(q: ProportionBasicQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      ...
      <div class="mode-grid">
        <button
          v-for="m in PROPORTION_BASIC_MODES"
          :key="m.id"
          type="button"
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ proportionBasicTopicLabel() }} · {{ proportionBasicDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ proportionBasicTopicLabel() }} · {{ proportionBasicDifficultyLabel(selectedDifficulty!) }}</span>
      ...
      <div v-if="test.currentQuestion?.pies?.length" class="da-pies">
        <DataAnalysisProportionPieChart
          v-for="(pie, pi) in test.currentQuestion.pies"
          :key="pi"
          :pie="pie"
        />
      </div>
    ...
    <template v-else-if="test.phase === 'summary'">
      ...
      >
        <div v-if="detailRow" class="da-result-detail">
          ...
          <div v-if="detailRow.question.pies?.length" class="da-pies">
            <DataAnalysisProportionPieChart
              v-for="(pie, pi) in detailRow.question.pies"
              :key="'d' + pi"
              :pie="pie"
            />
          </div>
