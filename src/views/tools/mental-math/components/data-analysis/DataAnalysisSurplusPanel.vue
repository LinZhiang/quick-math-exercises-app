...
import {
  useDataAnalysisSurplusTest,
  type SurplusResultRow,
} from '@/composables/data-analysis/useDataAnalysisSurplusTest'
import { isAiChatConfigured } from '@/services/deepseek'
import {
  SURPLUS_MODES,
  surplusDifficultyLabel,
  surplusTopicLabel,
  renderSurplusPassageHtml,
...
const selectedDifficulty = ref<SurplusDifficulty | null>(null)
const test = reactive(useDataAnalysisSurplusTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<SurplusResultRow | null>(null)
...
function selectDifficulty(d: SurplusDifficulty) {
...
function openResultDetail(row: SurplusResultRow) {
  detailRow.value = row
...
function optionClass(row: SurplusResultRow, idx: number) {
...
function plainHtml(text: string): string {
  return renderSurplusPassageHtml(text, [], false, text)
}
...
function passageHtml(q: SurplusQuestion | null | undefined) {
  if (!q?.passage) return ''
  return plainHtml(q.passage)
}

function stemHtml(q: SurplusQuestion | null | undefined) {
  if (!q?.stem) return ''
  return mathHtml(q.stem)
}

function explainHtml(q: SurplusQuestion | null | undefined) {
...
>
      ...
      <div class="mode-grid">
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`data-analysis-surplus-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ surplusTopicLabel() }} · {{ surplusDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ surplusTopicLabel() }} · {{ surplusDifficultyLabel(selectedDifficulty!) }}</span>
