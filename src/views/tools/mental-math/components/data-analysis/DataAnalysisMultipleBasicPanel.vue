...
import {
  useDataAnalysisMultipleBasicTest,
  type MultipleBasicResultRow,
} from '@/composables/data-analysis/useDataAnalysisMultipleBasicTest'
import { isAiChatConfigured } from '@/services/deepseek'
import {
  MULTIPLE_BASIC_MODES,
  multipleBasicDifficultyLabel,
  multipleBasicTopicLabel,
  renderMultipleBasicPassageHtml,
...
const selectedDifficulty = ref<MultipleBasicDifficulty | null>(null)
const test = reactive(useDataAnalysisMultipleBasicTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<MultipleBasicResultRow | null>(null)
...
function selectDifficulty(d: MultipleBasicDifficulty) {
...
function openResultDetail(row: MultipleBasicResultRow) {
  detailRow.value = row
...
function optionClass(row: MultipleBasicResultRow, idx: number) {
...
function plainHtml(text: string): string {
  return renderMultipleBasicPassageHtml(text, [], false, text)
}
...
function passageHtml(q: MultipleBasicQuestion | null | undefined) {
  if (!q?.passage) return ''
  return plainHtml(q.passage)
}

function stemHtml(q: MultipleBasicQuestion | null | undefined) {
  if (!q?.stem) return ''
  return mathHtml(q.stem)
}

function explainHtml(q: MultipleBasicQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「倍数与翻番——基本公式」：是几倍/多几倍/翻番。简单题纯文字；复杂题含数据表（先人均再比倍等），难度对齐教材或更高。本模块
        <strong>仅豆包</strong>出题。每轮
        {{ test.questionCount }} 题四选一。正计时，提交后暂停并公布答案（不另做数字高亮，以解析为准）。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in MULTIPLE_BASIC_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`data-analysis-multiple-basic-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ multipleBasicTopicLabel() }} · {{ multipleBasicDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ multipleBasicTopicLabel() }} · {{ multipleBasicDifficultyLabel(selectedDifficulty!) }}</span>
