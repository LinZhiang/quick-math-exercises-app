...
import {
  useDataAnalysisGrowthAvgAnnualTest,
  type GrowthAvgAnnualResultRow,
} from '@/composables/data-analysis/useDataAnalysisGrowthAvgAnnualTest'
import { isAiChatConfigured } from '@/services/deepseek'
import {
  GROWTH_AVG_ANNUAL_MODES,
  growthAvgAnnualDifficultyLabel,
  growthAvgAnnualTopicLabel,
  renderGrowthAvgAnnualPassageHtml,
...
import DataAnalysisGrowthChart from '@/views/tools/mental-math/components/data-analysis/DataAnalysisGrowthChart.vue'
import DataAnalysisGrowthAvgAnnualTable from '@/views/tools/mental-math/components/data-analysis/DataAnalysisGrowthAvgAnnualTable.vue'
...
const detailRow = ref<GrowthAvgAnnualResultRow | null>(null)
...
function selectDifficulty(d: GrowthAvgAnnualDifficulty) {
...
function openResultDetail(row: GrowthAvgAnnualResultRow) {
  detailRow.value = row
...
function optionClass(row: GrowthAvgAnnualResultRow, idx: number) {
...
function plainHtml(text: string): string {
  return renderGrowthAvgAnnualPassageHtml(text, [], false, text)
}
...
function passageHtml(q: GrowthAvgAnnualQuestion | null | undefined) {
  if (!q?.passage) return ''
  return plainHtml(q.passage)
}

function stemHtml(q: GrowthAvgAnnualQuestion | null | undefined) {
  if (!q?.stem) return ''
  return mathHtml(q.stem)
}

function explainHtml(q: GrowthAvgAnnualQuestion | null | undefined) {
  if (!q?.explanation) return ''
  return mathHtml(q.explanation)
}
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「增长——年均增长」：年均增长量/增长率、多期连乘、初期判定（一般/这n年/五年规划）、特征数字与近似估、(1+r)^n
        外推等。简单题纯文字、计算相对简明；复杂题为柱折线图或数据表，难度对齐真题。本模块
        <strong>仅豆包</strong>出题。每轮
        {{ test.questionCount }} 题四选一。正计时，提交后暂停并公布答案（不另做数字高亮，以解析为准）。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in GROWTH_AVG_ANNUAL_MODES"
          :key="m.id"
          type="button"
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ growthAvgAnnualTopicLabel() }} · {{ growthAvgAnnualDifficultyLabel(selectedDifficulty) }}。每轮
        {{ test.questionCount }} 题 · 豆包出题。正计时，提交后暂停看答案。
      </p>
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ growthAvgAnnualTopicLabel() }} · {{ growthAvgAnnualDifficultyLabel(selectedDifficulty!) }}</span>
      ...
      <DataAnalysisGrowthChart
        v-if="test.currentQuestion?.chart"
        :chart="test.currentQuestion.chart"
      />
      <DataAnalysisGrowthAvgAnnualTable
        v-if="test.currentQuestion?.table"
        :table="test.currentQuestion.table"
      />
    ...
    <template v-else-if="test.phase === 'summary'">
      ...
      >
        <div v-if="detailRow" class="da-result-detail">
          ...
          <section v-if="detailRow.question.chart">
            <h4>统计图</h4>
            <DataAnalysisGrowthChart :chart="detailRow.question.chart" />
          </section>
          <section v-if="detailRow.question.table">
            <h4>数据表</h4>
            <DataAnalysisGrowthAvgAnnualTable :table="detailRow.question.table" />
          </section>
          <section>
            <h4>题干</h4>
            <!-- eslint-disable-next-line vue/no-v-html -->
