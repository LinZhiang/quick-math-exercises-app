...
import {
  ...
  type MultipleBaseResultRow,
} from '@/composables/data-analysis/useDataAnalysisMultipleBaseTest'
import { isAiChatConfigured } from '@/services/deepseek'
import {
  MULTIPLE_BASE_MODES,
  multipleBaseDifficultyLabel,
  multipleBaseTopicLabel,
  renderMultipleBasePassageHtml,
...
const selectedDifficulty = ref<MultipleBaseDifficulty | null>(null)
const test = reactive(useDataAnalysisMultipleBaseTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<MultipleBaseResultRow | null>(null)
...
function openResultDetail(row: MultipleBaseResultRow) {
  detailRow.value = row
...
function optionClass(row: MultipleBaseResultRow, idx: number) {
...
function plainHtml(text: string): string {
  return renderMultipleBasePassageHtml(text, [], false, text)
}
...
function passageHtml(q: MultipleBaseQuestion | null | undefined) {
  if (!q?.passage) return ''
  return plainHtml(q.passage)
}

function stemHtml(q: MultipleBaseQuestion | null | undefined) {
  if (!q?.stem) return ''
  return mathHtml(q.stem)
}

function explainHtml(q: MultipleBaseQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「倍数与翻番——基期与增长量倍数」：基期倍数、增长量倍数。简单题纯文字；复杂题含数据表，难度高于教材。本模块
        <strong>仅豆包</strong>出题。每轮
        {{ test.questionCount }} 题四选一。正计时，提交后暂停并公布答案（不另做数字高亮，以解析为准）。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in MULTIPLE_BASE_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`data-analysis-multiple-base-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ multipleBaseTopicLabel() }} · {{ multipleBaseDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ multipleBaseTopicLabel() }} · {{ multipleBaseDifficultyLabel(selectedDifficulty!) }}</span>
