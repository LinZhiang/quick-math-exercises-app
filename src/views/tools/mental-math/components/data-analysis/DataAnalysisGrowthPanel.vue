...
import {
  useDataAnalysisGrowthTest,
  type GrowthGeneralResultRow,
} from '@/composables/data-analysis/useDataAnalysisGrowthTest'
import { isAiChatConfigured } from '@/services/deepseek'
import {
  GROWTH_GENERAL_MODES,
  growthGeneralDifficultyLabel,
  growthGeneralTopicLabel,
  renderGrowthPassageHtml,
...
const detailRow = ref<GrowthGeneralResultRow | null>(null)
...
function selectDifficulty(d: GrowthGeneralDifficulty) {
...
function openResultDetail(row: GrowthGeneralResultRow) {
  detailRow.value = row
...
function optionClass(row: GrowthGeneralResultRow, idx: number) {
...
function plainHtml(text: string): string {
  return renderGrowthPassageHtml(text, [], false, text)
}
...
function passageHtml(q: GrowthGeneralQuestion | null | undefined) {
  if (!q?.passage) return ''
  return plainHtml(q.passage)
}

function stemHtml(q: GrowthGeneralQuestion | null | undefined) {
  if (!q?.stem) return ''
  return mathHtml(q.stem)
}

function explainHtml(q: GrowthGeneralQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「增长——一般增长」：基期/现期、增长量、增长率、同比与环比。本模块
        <strong>仅豆包</strong>出题（复杂题含柱状+折线统计图）。每轮
        {{ test.questionCount }} 题四选一。正计时，提交后暂停并公布答案（不另做数字高亮，以解析为准）。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in GROWTH_GENERAL_MODES"
          :key="m.id"
          type="button"
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ growthGeneralTopicLabel() }} · {{ growthGeneralDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ growthGeneralTopicLabel() }} · {{ growthGeneralDifficultyLabel(selectedDifficulty!) }}</span>
      ...
      />
      <DataAnalysisGrowthChart
        v-if="test.currentQuestion?.secondaryChart"
        :chart="test.currentQuestion.secondaryChart"
      />
      ...
      >
        <div v-if="detailRow" class="da-result-detail">
          ...
          <section v-if="detailRow.question.chart">
            ...
            <DataAnalysisGrowthChart
              v-if="detailRow.question.secondaryChart"
              :chart="detailRow.question.secondaryChart"
            />
          </section>
          <section>
            <h4>题干</h4>
