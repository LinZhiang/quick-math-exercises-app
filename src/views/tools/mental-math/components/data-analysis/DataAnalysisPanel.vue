...
} from '@/composables/data-analysis/useDataAnalysisTest'
...
import {
  ...
  resolveDataAnalysisExplainHighlightSpans,
  resolveDataAnalysisStemHighlightSpans,
...
import MentalMathFavoriteButton from '@/views/tools/mental-math/components/shared/MentalMathFavoriteButton.vue'
...
function openResultDetail(row: DataAnalysisResultRow) {
  detailRow.value = row
...
function optionClass(row: DataAnalysisResultRow, idx: number) {
...
function passageHtml(q: DataAnalysisQuestion | null | undefined, highlight: boolean) {
  if (!q?.passage) return ''
  return renderDataAnalysisPassageHtml(q.passage, q.evidenceSpans ?? [], highlight, q.passage)
}

function stemHtml(q: DataAnalysisQuestion | null | undefined, highlight: boolean) {
  if (!q?.stem) return ''
  const spans = resolveDataAnalysisStemHighlightSpans(q.stem, q.passage, q.evidenceSpans ?? [])
  return renderDataAnalysisPassageHtml(q.stem, spans, highlight, q.passage || q.stem)
}

function explainHtml(q: DataAnalysisQuestion | null | undefined, highlight: boolean) {
  if (!q?.explanation) return ''
  const spans = resolveDataAnalysisExplainHighlightSpans(
    q.explanation,
    q.passage,
    q.stem,
    q.evidenceSpans ?? [],
  )
  return renderDataAnalysisPassageHtml(q.explanation, spans, highlight, q.passage || q.explanation)
}
...
watch(selectedDifficulty, () => {
  /* difficulty change clears paper via selectDifficulty */
})
</script>
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        ...
        {{ test.questionCount }} 题。错题集与「增长·一般增长」分开。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in DATA_ANALYSIS_MODES"
          :key="m.id"
          type="button"
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ dataAnalysisTopicLabel() }} · {{ dataAnalysisDifficultyLabel(selectedDifficulty) }}。每轮
        {{ test.questionCount }} 题。正计时，提交后暂停看答案。
      </p>
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ dataAnalysisTopicLabel() }} · {{ dataAnalysisDifficultyLabel(selectedDifficulty!) }}</span>
      ...
      <div v-if="test.currentQuestion?.passage" class="da-passage">
        ...
        <p
          class="da-passage__body"
          :class="{ 'da-passage__body--revealed': test.submitted }"
          v-html="passageHtml(test.currentQuestion, test.submitted)"
        />
        <p
          v-if="test.submitted && (test.currentQuestion.evidenceSpans?.length ?? 0) > 0"
          class="da-passage__hint"
        >
          高亮：主体分色 · 数据用下划线标出
        </p>
      </div>
      ...
      <div v-if="test.currentQuestion" class="chinese-quiz__stem">
        <!-- eslint-disable-next-line vue/no-v-html -->
        <p
          class="chinese-quiz__question"
          :class="{ 'da-passage__body--revealed': test.submitted }"
          v-html="stemHtml(test.currentQuestion, test.submitted)"
        />
      </div>
      ...
      <div v-if="test.submitted && test.currentQuestion" class="chinese-quiz__feedback">
        ...
        <p
          ...
          v-html="explainHtml(test.currentQuestion, true)"
        />
        <div
        ...
        <div v-if="detailRow" class="da-result-detail">
          ...
          <section v-if="detailRow.question.passage">
            ...
            <p
              ...
              v-html="passageHtml(detailRow.question, true)"
            />
          </section>
          <section>
            <h4>题干</h4>
            ...
            <p
              ...
              v-html="stemHtml(detailRow.question, true)"
            />
          </section>
          <section>
            <h4>选项</h4>
          ...
          <section v-if="detailRow.question.method || detailRow.question.explanation">
            ...
            <p
              ...
              v-html="explainHtml(detailRow.question, true)"
            />
          </section>
        </div>
        <template #footer>
