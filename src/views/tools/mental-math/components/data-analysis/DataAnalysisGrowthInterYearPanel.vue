...
import {
  useDataAnalysisGrowthInterYearTest,
  type GrowthInterYearResultRow,
} from '@/composables/data-analysis/useDataAnalysisGrowthInterYearTest'
import { isAiChatConfigured } from '@/services/deepseek'
import {
  GROWTH_INTER_YEAR_MODES,
  growthInterYearDifficultyLabel,
  growthInterYearTopicLabel,
  renderGrowthInterYearPassageHtml,
...
import DataAnalysisGrowthInterYearChart from '@/views/tools/mental-math/components/data-analysis/DataAnalysisGrowthInterYearChart.vue'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<GrowthInterYearDifficulty | null>(null)
const test = reactive(useDataAnalysisGrowthInterYearTest(selectedDifficulty))
...
function selectDifficulty(d: GrowthInterYearDifficulty) {
...
function openResultDetail(row: GrowthInterYearResultRow) {
  detailRow.value = row
...
function optionClass(row: GrowthInterYearResultRow, idx: number) {
...
function plainHtml(text: string): string {
  return renderGrowthInterYearPassageHtml(text, [], false, text)
}
...
function passageHtml(q: GrowthInterYearQuestion | null | undefined) {
  if (!q?.passage) return ''
  return plainHtml(q.passage)
}

function stemHtml(q: GrowthInterYearQuestion | null | undefined) {
  if (!q?.stem) return ''
  return mathHtml(q.stem)
}

function explainHtml(q: GrowthInterYearQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      ...
      <div class="mode-grid">
        <button
          v-for="m in GROWTH_INTER_YEAR_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`data-analysis-growth-inter-year-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ growthInterYearTopicLabel() }} · {{ growthInterYearDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ growthInterYearTopicLabel() }} · {{ growthInterYearDifficultyLabel(selectedDifficulty!) }}</span>
      ...
      <DataAnalysisGrowthInterYearChart
      ...
      >
        <div v-if="detailRow" class="da-result-detail">
          ...
          <section v-if="detailRow.question.chart">
            <h4>统计图</h4>
            <DataAnalysisGrowthInterYearChart :chart="detailRow.question.chart" />
          </section>
          <section>
            <h4>题干</h4>
            <!-- eslint-disable-next-line vue/no-v-html -->
