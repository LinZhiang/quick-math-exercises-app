...
import {
  useDataAnalysisAverageBaseTest,
  type AverageBaseResultRow,
} from '@/composables/data-analysis/useDataAnalysisAverageBaseTest'
import { isAiChatConfigured } from '@/services/deepseek'
import {
  AVERAGE_BASE_MODES,
  averageBaseDifficultyLabel,
  averageBaseTopicLabel,
  renderAverageBasePassageHtml,
...
const selectedDifficulty = ref<AverageBaseDifficulty | null>(null)
const test = reactive(useDataAnalysisAverageBaseTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<AverageBaseResultRow | null>(null)
...
function selectDifficulty(d: AverageBaseDifficulty) {
...
function openResultDetail(row: AverageBaseResultRow) {
  detailRow.value = row
...
function optionClass(row: AverageBaseResultRow, idx: number) {
...
function plainHtml(text: string): string {
  return renderAverageBasePassageHtml(text, [], false, text)
}
...
function passageHtml(q: AverageBaseQuestion | null | undefined) {
  if (!q?.passage) return ''
  return plainHtml(q.passage)
}

function stemHtml(q: AverageBaseQuestion | null | undefined) {
  if (!q?.stem) return ''
  return mathHtml(q.stem)
}

function explainHtml(q: AverageBaseQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「平均数——基期平均数」：基期平均数、平均数增长量/增长率与升降判断。简单/复杂均为纯文字；复杂对齐教材货运/快递真题。本模块
        <strong>仅豆包</strong>出题。每轮
        {{ test.questionCount }} 题四选一。正计时，提交后暂停并公布答案（不另做数字高亮，以解析为准）。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in AVERAGE_BASE_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`data-analysis-average-base-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ averageBaseTopicLabel() }} · {{ averageBaseDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ averageBaseTopicLabel() }} · {{ averageBaseDifficultyLabel(selectedDifficulty!) }}</span>
