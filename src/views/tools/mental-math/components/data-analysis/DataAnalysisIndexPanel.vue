...
import {
  useDataAnalysisIndexTest,
  type IndexResultRow,
} from '@/composables/data-analysis/useDataAnalysisIndexTest'
import { isAiChatConfigured } from '@/services/deepseek'
import {
  INDEX_MODES,
  indexDifficultyLabel,
  indexTopicLabel,
  renderIndexPassageHtml,
...
const selectedDifficulty = ref<IndexDifficulty | null>(null)
const test = reactive(useDataAnalysisIndexTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<IndexResultRow | null>(null)
...
function selectDifficulty(d: IndexDifficulty) {
...
function openResultDetail(row: IndexResultRow) {
  detailRow.value = row
...
function optionClass(row: IndexResultRow, idx: number) {
...
function plainHtml(text: string): string {
  return renderIndexPassageHtml(text, [], false, text)
}
...
function passageHtml(q: IndexQuestion | null | undefined) {
  if (!q?.passage) return ''
  return plainHtml(q.passage)
}

function stemHtml(q: IndexQuestion | null | undefined) {
  if (!q?.stem) return ''
  return mathHtml(q.stem)
}

function explainHtml(q: IndexQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「指数」：指数与实际值、增长率、倍数的关系；点数差与增长率差。简单/复杂均为纯文字无表；简单略易，复杂对齐教材景气指数题或更难。本模块
        <strong>仅豆包</strong>出题。每轮
        {{ test.questionCount }} 题四选一。正计时，提交后暂停并公布答案（不另做数字高亮，以解析为准）。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in INDEX_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`data-analysis-index-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ indexTopicLabel() }} · {{ indexDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ indexTopicLabel() }} · {{ indexDifficultyLabel(selectedDifficulty!) }}</span>
