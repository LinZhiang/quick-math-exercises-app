...
import {
  useDataAnalysisPullTest,
  type PullResultRow,
} from '@/composables/data-analysis/useDataAnalysisPullTest'
import { isAiChatConfigured } from '@/services/deepseek'
import {
  PULL_MODES,
  pullDifficultyLabel,
  pullTopicLabel,
  renderPullPassageHtml,
  type PullDifficulty,
  type PullQuestion,
...
const selectedDifficulty = ref<PullDifficulty | null>(null)
const test = reactive(useDataAnalysisPullTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<PullResultRow | null>(null)
...
function selectDifficulty(d: PullDifficulty) {
...
function openResultDetail(row: PullResultRow) {
  detailRow.value = row
...
function optionClass(row: PullResultRow, idx: number) {
...
function plainHtml(text: string): string {
  return renderPullPassageHtml(text, [], false, text)
}
...
function passageHtml(q: PullQuestion | null | undefined) {
  if (!q?.passage) return ''
  return plainHtml(q.passage)
}

function stemHtml(q: PullQuestion | null | undefined) {
  if (!q?.stem) return ''
  return mathHtml(q.stem)
}

function explainHtml(q: PullQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「拉动增长和比例」：拉动……增长、贡献率、利润率及其变化。简单/复杂均为纯文字无表；简单略易，复杂对齐教材拉动/贡献率/利润率真题或更难。本模块
        <strong>仅豆包</strong>出题。每轮
        {{ test.questionCount }} 题四选一。正计时，提交后暂停并公布答案（不另做数字高亮，以解析为准）。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in PULL_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`data-analysis-pull-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ pullTopicLabel() }} · {{ pullDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ pullTopicLabel() }} · {{ pullDifficultyLabel(selectedDifficulty!) }}</span>
