...
import {
  useDataAnalysisGrowthMixedTest,
  type GrowthMixedResultRow,
} from '@/composables/data-analysis/useDataAnalysisGrowthMixedTest'
import { isAiChatConfigured } from '@/services/deepseek'
import {
  GROWTH_MIXED_MODES,
  growthMixedDifficultyLabel,
  growthMixedTopicLabel,
  renderGrowthMixedPassageHtml,
...
const selectedDifficulty = ref<GrowthMixedDifficulty | null>(null)
const test = reactive(useDataAnalysisGrowthMixedTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<GrowthMixedResultRow | null>(null)
...
function selectDifficulty(d: GrowthMixedDifficulty) {
...
function openResultDetail(row: GrowthMixedResultRow) {
  detailRow.value = row
...
function optionClass(row: GrowthMixedResultRow, idx: number) {
...
function plainHtml(text: string): string {
  return renderGrowthMixedPassageHtml(text, [], false, text)
}
...
function passageHtml(q: GrowthMixedQuestion | null | undefined) {
  if (!q?.passage) return ''
  return plainHtml(q.passage)
}

function stemHtml(q: GrowthMixedQuestion | null | undefined) {
  if (!q?.stem) return ''
  return mathHtml(q.stem)
}

function explainHtml(q: GrowthMixedQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「增长——混合增长」：整体增速介于部分之间、偏向基期较大部分、十字交叉法、精确混合公式、反推另一部分增速等。简单/复杂均为纯文字；复杂题选项贴近，须十字交叉/精确算/估权，对齐书上难题。本模块
        <strong>仅豆包</strong>出题。每轮
        {{ test.questionCount }} 题四选一。正计时，提交后暂停并公布答案（不另做数字高亮，以解析为准）。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in GROWTH_MIXED_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`data-analysis-growth-mixed-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ growthMixedTopicLabel() }} · {{ growthMixedDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ growthMixedTopicLabel() }} · {{ growthMixedDifficultyLabel(selectedDifficulty!) }}</span>
