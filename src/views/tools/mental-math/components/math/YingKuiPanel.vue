...
import {
  useYingKuiTest,
  type YingKuiResultRow,
} from '@/composables/math/useYingKuiTest'
import {
  YING_KUI_MODES,
...
const selectedDifficulty = ref<YingKuiDifficulty | null>(null)
const test = reactive(useYingKuiTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<YingKuiResultRow | null>(null)
...
function selectDifficulty(d: YingKuiDifficulty) {
...
function openResultDetail(row: YingKuiResultRow) {
  detailRow.value = row
...
function optionClass(row: YingKuiResultRow, idx: number) {
...
function passageHtml(q: YingKuiQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: YingKuiQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: YingKuiQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「其他运算·盈亏问题」：一盈一尽、一亏一尽、一盈一亏、两次皆盈/皆亏。对象数=盈亏差÷分配差。简单比经典真题更易；普通对齐经典真题；困难为更高阶变式。本地组卷，每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in YING_KUI_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-other-ying-kui-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ yingKuiTopicLabel() }} · {{ yingKuiDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ yingKuiTopicLabel() }} · {{ yingKuiDifficultyLabel(selectedDifficulty!) }}</span>
