...
import {
  useRatioMethodTest,
  type RatioMethodResultRow,
} from '@/composables/math/useRatioMethodTest'
import {
  RATIO_METHOD_MODES,
  ratioMethodDifficultyLabel,
  ratioMethodTopicLabel,
  type RatioMethodDifficulty,
  type RatioMethodQuestion,
} from '@/utils/math/ratioMethodPractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<RatioMethodDifficulty | null>(null)
const test = reactive(useRatioMethodTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<RatioMethodResultRow | null>(null)
...
function selectDifficulty(d: RatioMethodDifficulty) {
...
function openResultDetail(row: RatioMethodResultRow) {
  detailRow.value = row
...
function optionClass(row: RatioMethodResultRow, idx: number) {
...
function passageHtml(q: RatioMethodQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: RatioMethodQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: RatioMethodQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「比例法」：设份数代替未知数；普通对齐经典真题 1（比例统一）；困难对齐经典真题 2 行程比例变式。本地程序组卷（不调用
        AI），每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in RATIO_METHOD_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-skill-ratio-method-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ ratioMethodTopicLabel() }} · {{ ratioMethodDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ ratioMethodTopicLabel() }} · {{ ratioMethodDifficultyLabel(selectedDifficulty!) }}</span>
