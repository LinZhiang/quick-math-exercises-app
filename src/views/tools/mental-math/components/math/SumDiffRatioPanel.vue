...
import {
  useSumDiffRatioTest,
  type SumDiffRatioResultRow,
} from '@/composables/math/useSumDiffRatioTest'
import {
  SUM_DIFF_RATIO_MODES,
  sumDiffRatioDifficultyLabel,
  sumDiffRatioTopicLabel,
  type SumDiffRatioDifficulty,
  type SumDiffRatioQuestion,
} from '@/utils/math/sumDiffRatioPractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<SumDiffRatioDifficulty | null>(null)
const test = reactive(useSumDiffRatioTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<SumDiffRatioResultRow | null>(null)
...
function selectDifficulty(d: SumDiffRatioDifficulty) {
...
function openResultDetail(row: SumDiffRatioResultRow) {
  detailRow.value = row
...
function optionClass(row: SumDiffRatioResultRow, idx: number) {
...
function passageHtml(q: SumDiffRatioQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: SumDiffRatioQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: SumDiffRatioQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「和差倍比问题」：找等量关系列方程；简单对齐经典真题 1，普通对齐经典真题 2；困难为更高阶变式。本地程序组卷（不调用
        AI），每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in SUM_DIFF_RATIO_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-highfreq-sum-diff-ratio-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ sumDiffRatioTopicLabel() }} · {{ sumDiffRatioDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ sumDiffRatioTopicLabel() }} · {{ sumDiffRatioDifficultyLabel(selectedDifficulty!) }}</span>
