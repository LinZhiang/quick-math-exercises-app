...
import {
  usePermCombConstraintTest,
  type PermCombConstraintResultRow,
} from '@/composables/math/usePermCombConstraintTest'
import {
  PERM_COMB_CONSTRAINT_MODES,
  permCombConstraintDifficultyLabel,
  permCombConstraintTopicLabel,
  type PermCombConstraintDifficulty,
  type PermCombConstraintQuestion,
} from '@/utils/math/permCombConstraintPractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<PermCombConstraintDifficulty | null>(null)
const test = reactive(usePermCombConstraintTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<PermCombConstraintResultRow | null>(null)
...
function selectDifficulty(d: PermCombConstraintDifficulty) {
...
function openResultDetail(row: PermCombConstraintResultRow) {
  detailRow.value = row
...
function optionClass(row: PermCombConstraintResultRow, idx: number) {
...
function passageHtml(q: PermCombConstraintQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: PermCombConstraintQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: PermCombConstraintQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「排列组合·限制条件型」：优限法、捆绑法、插空法、归一法；环形与错位。简单对齐真题 3、4；普通对齐真题 5、6。本地程序组卷（不调用
        AI），每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in PERM_COMB_CONSTRAINT_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-highfreq-perm-comb-constraint-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ permCombConstraintTopicLabel() }} · {{ permCombConstraintDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ permCombConstraintTopicLabel() }} · {{ permCombConstraintDifficultyLabel(selectedDifficulty!) }}</span>
