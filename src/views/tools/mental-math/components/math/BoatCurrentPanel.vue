...
import {
  useBoatCurrentTest,
  type BoatCurrentResultRow,
} from '@/composables/math/useBoatCurrentTest'
import {
  BOAT_CURRENT_MODES,
  boatCurrentDifficultyLabel,
  boatCurrentTopicLabel,
  type BoatCurrentDifficulty,
  type BoatCurrentQuestion,
} from '@/utils/math/boatCurrentPractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<BoatCurrentDifficulty | null>(null)
const test = reactive(useBoatCurrentTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<BoatCurrentResultRow | null>(null)
...
function selectDifficulty(d: BoatCurrentDifficulty) {
...
function openResultDetail(row: BoatCurrentResultRow) {
  detailRow.value = row
...
function optionClass(row: BoatCurrentResultRow, idx: number) {
...
function passageHtml(q: BoatCurrentQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: BoatCurrentQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: BoatCurrentQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「流水行船问题」：顺水=船速+水速，逆水=船速−水速。简单对齐经典真题 8（往返时差求水速再算另一船）；仅简单/困难两档。本地程序组卷（不调用
        AI），每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in BOAT_CURRENT_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-highfreq-boat-current-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ boatCurrentTopicLabel() }} · {{ boatCurrentDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ boatCurrentTopicLabel() }} · {{ boatCurrentDifficultyLabel(selectedDifficulty!) }}</span>
