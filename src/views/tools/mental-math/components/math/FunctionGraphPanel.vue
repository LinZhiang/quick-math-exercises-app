...
import {
  useFunctionGraphTest,
  type FunctionGraphResultRow,
} from '@/composables/math/useFunctionGraphTest'
import {
  FUNCTION_GRAPH_MODES,
  functionGraphDifficultyLabel,
  functionGraphTopicLabel,
...
import { FUNCTION_GRAPH_KIND_LABEL } from '@/utils/math/functionGraphPractice'

const selectedDifficulty = ref<FunctionGraphDifficulty | null>(null)
const test = reactive(useFunctionGraphTest(selectedDifficulty))
...
function openResultDetail(row: FunctionGraphResultRow) {
  detailRow.value = row
...
function optionClass(row: FunctionGraphResultRow, idx: number) {
...
function passageHtml(q: FunctionGraphQuestion | null | undefined) {
  if (!q?.passage) return ''
  return mathHtml(q.passage)
}

function stemHtml(q: FunctionGraphQuestion | null | undefined) {
  if (!q?.stem) return ''
  return mathHtml(q.stem)
}

function explainHtml(q: FunctionGraphQuestion | null | undefined) {
  if (!q?.explanation) return ''
  return methodHtml(q.explanation)
}
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      ...
      <div class="mode-grid">
        <button
          v-for="m in FUNCTION_GRAPH_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-other-function-graph-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ functionGraphTopicLabel() }} · {{ functionGraphDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ functionGraphTopicLabel() }} · {{ functionGraphDifficultyLabel(selectedDifficulty!) }}</span>
      ...
      <div v-if="test.currentQuestion" class="fg-options">
        <button
          v-for="(kind, idx) in test.currentQuestion.optionKinds"
          :key="idx"
          type="button"
          class="fg-option"
          :class="{
        ...
        >
          <span class="fg-option__key">{{ 'ABCD'[Number(idx)] }}</span>
          <FunctionGraphCurveView :kind="kind" compact />
        </button>
      </div>
      ...
      <div v-if="test.submitted && test.currentQuestion" class="chinese-quiz__feedback">
        ...
        >
          ...
          <template v-else>
            回答错误 · 正确答案：
            {{ 'ABCD'[test.currentQuestion.correctIndex] }}（{{
              FUNCTION_GRAPH_KIND_LABEL[test.currentQuestion.optionKinds[test.currentQuestion.correctIndex]!]
            }}）
          </template>
        </p>
    ...
    <template v-else-if="test.phase === 'summary'">
      ...
      >
        <div v-if="detailRow" class="da-result-detail">
          ...
          <section>
            <h4>选项</h4>
            <div class="fg-options fg-options--detail">
              <div
                v-for="(kind, idx) in detailRow.question.optionKinds"
                :key="idx"
                class="fg-option"
