...
import {
  useSequenceTest,
  type SequenceResultRow,
} from '@/composables/math/useSequenceTest'
import {
  SEQUENCE_MODES,
  sequenceDifficultyLabel,
  sequenceTopicLabel,
  type SequenceDifficulty,
  type SequenceQuestion,
} from '@/utils/math/sequencePractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<SequenceDifficulty | null>(null)
const test = reactive(useSequenceTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<SequenceResultRow | null>(null)
...
function selectDifficulty(d: SequenceDifficulty) {
...
function openResultDetail(row: SequenceResultRow) {
  detailRow.value = row
...
function optionClass(row: SequenceResultRow, idx: number) {
...
function passageHtml(q: SequenceQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: SequenceQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: SequenceQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「其他运算·数列问题」：等差与等比的通项、对称、求和。简单比真题略易；普通对齐真题 1、2；困难为更高阶变式。本地组卷，每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in SEQUENCE_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-other-sequence-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ sequenceTopicLabel() }} · {{ sequenceDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ sequenceTopicLabel() }} · {{ sequenceDifficultyLabel(selectedDifficulty!) }}</span>
