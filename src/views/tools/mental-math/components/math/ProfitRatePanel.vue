...
import {
  useProfitRateTest,
  type ProfitRateResultRow,
} from '@/composables/math/useProfitRateTest'
import {
  PROFIT_RATE_MODES,
...
} from '@/utils/math/profitRatePractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<ProfitRateDifficulty | null>(null)
const test = reactive(useProfitRateTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<ProfitRateResultRow | null>(null)
...
function selectDifficulty(d: ProfitRateDifficulty) {
...
function openResultDetail(row: ProfitRateResultRow) {
  detailRow.value = row
...
function optionClass(row: ProfitRateResultRow, idx: number) {
...
function passageHtml(q: ProfitRateQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: ProfitRateQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: ProfitRateQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「利润率计算」：利润率=利润/成本=(售价−成本)/成本=售价/成本−1。对齐经典真题 2（成本/售价变动与百分点）；答案为利润率或百分点，不考「赚了多少元」。仅简单/困难两档。本地程序组卷（不调用
        AI），每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in PROFIT_RATE_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-highfreq-profit-rate-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ profitRateTopicLabel() }} · {{ profitRateDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ profitRateTopicLabel() }} · {{ profitRateDifficultyLabel(selectedDifficulty!) }}</span>
