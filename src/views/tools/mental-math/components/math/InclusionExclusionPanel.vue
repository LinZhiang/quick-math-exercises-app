...
import {
  useInclusionExclusionTest,
  type InclusionExclusionResultRow,
} from '@/composables/math/useInclusionExclusionTest'
import {
  INCLUSION_EXCLUSION_MODES,
  inclusionExclusionDifficultyLabel,
  inclusionExclusionTopicLabel,
  type InclusionExclusionDifficulty,
  type InclusionExclusionQuestion,
} from '@/utils/math/inclusionExclusionPractice'
...
const selectedDifficulty = ref<InclusionExclusionDifficulty | null>(null)
const test = reactive(useInclusionExclusionTest(selectedDifficulty))
...
function selectDifficulty(d: InclusionExclusionDifficulty) {
...
function openResultDetail(row: InclusionExclusionResultRow) {
  detailRow.value = row
...
function optionClass(row: InclusionExclusionResultRow, idx: number) {
...
function passageHtml(q: InclusionExclusionQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: InclusionExclusionQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: InclusionExclusionQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      ...
      <div class="mode-grid">
        <button
          v-for="m in INCLUSION_EXCLUSION_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-other-inclusion-exclusion-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ inclusionExclusionTopicLabel() }} · {{ inclusionExclusionDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ inclusionExclusionTopicLabel() }} · {{ inclusionExclusionDifficultyLabel(selectedDifficulty!) }}</span>
      ...
      <div v-if="test.submitted && test.currentQuestion" class="chinese-quiz__feedback">
        ...
        />
        <InclusionExclusionVennDiagram
          v-if="test.currentQuestion?.diagramPreset"
          :preset="test.currentQuestion.diagramPreset"
          :caption="test.currentQuestion.diagramCaption"
        />
        <div
      ...
      >
        <div v-if="detailRow" class="da-result-detail">
          ...
          <section v-if="detailRow.question.method || detailRow.question.explanation">
            ...
            />
            <InclusionExclusionVennDiagram
              v-if="detailRow.question.diagramPreset"
              :preset="detailRow.question.diagramPreset"
              :caption="detailRow.question.diagramCaption"
            />
          </section>
        </div>
        <template #footer>
