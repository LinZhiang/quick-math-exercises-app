...
import {
  useEquationMethodTest,
  type EqMethodResultRow,
} from '@/composables/math/useEquationMethodTest'
import {
  EQ_METHOD_MODES,
  eqMethodDifficultyLabel,
  eqMethodTopicLabel,
  type EqMethodDifficulty,
  type EqMethodQuestion,
} from '@/utils/math/equationMethodPractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<EqMethodDifficulty | null>(null)
const test = reactive(useEquationMethodTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<EqMethodResultRow | null>(null)
...
function selectDifficulty(d: EqMethodDifficulty) {
...
function openResultDetail(row: EqMethodResultRow) {
  detailRow.value = row
...
function optionClass(row: EqMethodResultRow, idx: number) {
...
function passageHtml(q: EqMethodQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: EqMethodQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: EqMethodQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「方程法」：设未知数列方程；普通对齐经典真题 1/2；困难对齐经典真题 3 整体代换变式。本地程序组卷（不调用
        AI），每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in EQ_METHOD_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-skill-eq-method-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ eqMethodTopicLabel() }} · {{ eqMethodDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ eqMethodTopicLabel() }} · {{ eqMethodDifficultyLabel(selectedDifficulty!) }}</span>
