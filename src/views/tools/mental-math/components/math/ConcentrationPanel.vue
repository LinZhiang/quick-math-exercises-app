...
import {
  useConcentrationTest,
  type ConcentrationResultRow,
} from '@/composables/math/useConcentrationTest'
import {
  CONCENTRATION_MODES,
  concentrationDifficultyLabel,
  concentrationTopicLabel,
  type ConcentrationDifficulty,
  type ConcentrationQuestion,
} from '@/utils/math/concentrationPractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<ConcentrationDifficulty | null>(null)
const test = reactive(useConcentrationTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<ConcentrationResultRow | null>(null)
...
function selectDifficulty(d: ConcentrationDifficulty) {
...
function openResultDetail(row: ConcentrationResultRow) {
  detailRow.value = row
...
function optionClass(row: ConcentrationResultRow, idx: number) {
...
function passageHtml(q: ConcentrationQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: ConcentrationQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: ConcentrationQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「浓度问题」：浓度=溶质÷溶液；溶液=溶质+溶剂；十字交叉混合；倒水加水溶质按比例保留。简单/普通/困难三档。本地程序组卷（不调用
        AI），每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in CONCENTRATION_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-highfreq-concentration-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ concentrationTopicLabel() }} · {{ concentrationDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ concentrationTopicLabel() }} · {{ concentrationDifficultyLabel(selectedDifficulty!) }}</span>
