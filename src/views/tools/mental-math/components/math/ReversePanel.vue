...
import {
  useReverseTest,
  type ReverseResultRow,
} from '@/composables/math/useReverseTest'
import {
  REVERSE_MODES,
  reverseDifficultyLabel,
  reverseTopicLabel,
  type ReverseDifficulty,
  type ReverseQuestion,
} from '@/utils/math/reversePractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<ReverseDifficulty | null>(null)
const test = reactive(useReverseTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<ReverseResultRow | null>(null)
...
function selectDifficulty(d: ReverseDifficulty) {
...
function openResultDetail(row: ReverseResultRow) {
  detailRow.value = row
...
function optionClass(row: ReverseResultRow, idx: number) {
...
function passageHtml(q: ReverseQuestion | null | undefined) {
  if (!q?.passage) return ''
  return mathHtml(q.passage)
}

function stemHtml(q: ReverseQuestion | null | undefined) {
  if (!q?.stem) return ''
  return mathHtml(q.stem)
}

function explainHtml(q: ReverseQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「其他运算·逆推问题」：从结果分步还原；分数剩量 ± 常数用书中逆推公式。简单比经典真题更易；普通对齐经典真题（多日修路）；困难为更高阶变式。本地组卷，每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in REVERSE_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-other-reverse-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ reverseTopicLabel() }} · {{ reverseDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ reverseTopicLabel() }} · {{ reverseDifficultyLabel(selectedDifficulty!) }}</span>
