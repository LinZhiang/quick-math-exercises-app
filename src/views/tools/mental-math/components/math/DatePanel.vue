...
import {
  useDateTest,
  type DateResultRow,
} from '@/composables/math/useDateTest'
import {
  DATE_MODES,
  dateDifficultyLabel,
  dateTopicLabel,
  type DateDifficulty,
  type DateQuestion,
} from '@/utils/math/datePractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<DateDifficulty | null>(null)
const test = reactive(useDateTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<DateResultRow | null>(null)
...
function selectDifficulty(d: DateDifficulty) {
...
function openResultDetail(row: DateResultRow) {
  detailRow.value = row
...
function optionClass(row: DateResultRow, idx: number) {
...
function passageHtml(q: DateQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: DateQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: DateQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「其他运算·日期问题」：平闰年、星期推算、月历次数、轮值。简单对齐示例 1/2/3；普通对齐真题 1、2；困难为更高阶变式。本地组卷，每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-other-date-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ dateTopicLabel() }} · {{ dateDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ dateTopicLabel() }} · {{ dateDifficultyLabel(selectedDifficulty!) }}</span>
