...
import {
  useRightTriangleTest,
  type RightTriangleResultRow,
} from '@/composables/math/useRightTriangleTest'
import {
  RIGHT_TRIANGLE_MODES,
  rightTriangleDifficultyLabel,
  rightTriangleTopicLabel,
  type RightTriangleDifficulty,
  type RightTriangleQuestion,
} from '@/utils/math/rightTrianglePractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<RightTriangleDifficulty | null>(null)
const test = reactive(useRightTriangleTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<RightTriangleResultRow | null>(null)
...
function selectDifficulty(d: RightTriangleDifficulty) {
...
function openResultDetail(row: RightTriangleResultRow) {
  detailRow.value = row
...
function optionClass(row: RightTriangleResultRow, idx: number) {
...
function passageHtml(q: RightTriangleQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: RightTriangleQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: RightTriangleQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      ...
      <div class="mode-grid">
        <button
          v-for="m in RIGHT_TRIANGLE_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-highfreq-right-triangle-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ rightTriangleTopicLabel() }} · {{ rightTriangleDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ rightTriangleTopicLabel() }} · {{ rightTriangleDifficultyLabel(selectedDifficulty!) }}</span>
