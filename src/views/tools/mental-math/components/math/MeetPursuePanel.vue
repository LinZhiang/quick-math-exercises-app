...
import {
  useMeetPursueTest,
  type MeetPursueResultRow,
} from '@/composables/math/useMeetPursueTest'
import {
  MEET_PURSUE_MODES,
  meetPursueDifficultyLabel,
  meetPursueTopicLabel,
  type MeetPursueDifficulty,
  type MeetPursueQuestion,
} from '@/utils/math/meetPursuePractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<MeetPursueDifficulty | null>(null)
const test = reactive(useMeetPursueTest(selectedDifficulty))
...
function selectDifficulty(d: MeetPursueDifficulty) {
...
function openResultDetail(row: MeetPursueResultRow) {
  detailRow.value = row
...
function optionClass(row: MeetPursueResultRow, idx: number) {
...
function passageHtml(q: MeetPursueQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: MeetPursueQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: MeetPursueQuestion | null | undefined) {
...
>
      ...
      <div class="mode-grid">
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-highfreq-meet-pursue-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ meetPursueTopicLabel() }} · {{ meetPursueDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ meetPursueTopicLabel() }} · {{ meetPursueDifficultyLabel(selectedDifficulty!) }}</span>
