...
import {
  usePermCombClassicTest,
  type PermCombClassicResultRow,
} from '@/composables/math/usePermCombClassicTest'
import {
  PERM_COMB_CLASSIC_MODES,
  permCombClassicDifficultyLabel,
  permCombClassicTopicLabel,
  type PermCombClassicDifficulty,
  type PermCombClassicQuestion,
} from '@/utils/math/permCombClassicPractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<PermCombClassicDifficulty | null>(null)
const test = reactive(usePermCombClassicTest(selectedDifficulty))
...
function selectDifficulty(d: PermCombClassicDifficulty) {
...
function openResultDetail(row: PermCombClassicResultRow) {
  detailRow.value = row
...
function optionClass(row: PermCombClassicResultRow, idx: number) {
...
function passageHtml(q: PermCombClassicQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: PermCombClassicQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: PermCombClassicQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「排列组合·经典模型」：环形排列、错位重排、同素分堆（插板法）。简单对齐真题 7；普通对齐真题 8、9。本地程序组卷（不调用
        AI），每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in PERM_COMB_CLASSIC_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-highfreq-perm-comb-classic-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ permCombClassicTopicLabel() }} · {{ permCombClassicDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ permCombClassicTopicLabel() }} · {{ permCombClassicDifficultyLabel(selectedDifficulty!) }}</span>
