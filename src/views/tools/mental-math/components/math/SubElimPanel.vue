...
import {
  useSubElimTest,
  type SubElimResultRow,
} from '@/composables/math/useSubElimTest'
import {
  SUB_ELIM_MODES,
  subElimDifficultyLabel,
  subElimTopicLabel,
  type SubElimDifficulty,
  type SubElimQuestion,
} from '@/utils/math/subElimPractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<SubElimDifficulty | null>(null)
const test = reactive(useSubElimTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<SubElimResultRow | null>(null)
...
function selectDifficulty(d: SubElimDifficulty) {
...
function openResultDetail(row: SubElimResultRow) {
  detailRow.value = row
...
function optionClass(row: SubElimResultRow, idx: number) {
...
function passageHtml(q: SubElimQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: SubElimQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: SubElimQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「代入排除法」：直接/选择代入；普通对齐经典真题 1/2；困难为更难变式（每题题型不同）。本地程序组卷（不调用
        AI），每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in SUB_ELIM_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-skill-sub-elim-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ subElimTopicLabel() }} · {{ subElimDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ subElimTopicLabel() }} · {{ subElimDifficultyLabel(selectedDifficulty!) }}</span>
