...
import {
  useOrdinaryWorkTest,
  type OrdinaryWorkResultRow,
} from '@/composables/math/useOrdinaryWorkTest'
import {
  ORDINARY_WORK_MODES,
  ordinaryWorkDifficultyLabel,
  ordinaryWorkTopicLabel,
  type OrdinaryWorkDifficulty,
  type OrdinaryWorkQuestion,
} from '@/utils/math/ordinaryWorkPractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<OrdinaryWorkDifficulty | null>(null)
const test = reactive(useOrdinaryWorkTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<OrdinaryWorkResultRow | null>(null)
...
function selectDifficulty(d: OrdinaryWorkDifficulty) {
...
function openResultDetail(row: OrdinaryWorkResultRow) {
  detailRow.value = row
...
function optionClass(row: OrdinaryWorkResultRow, idx: number) {
...
function passageHtml(q: OrdinaryWorkQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: OrdinaryWorkQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: OrdinaryWorkQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「普通工程问题」：工作总量=效率×时间；同效/同时/同量时的正反比。简单对齐或略低于经典真题 1；仅简单/困难两档。本地程序组卷（不调用
        AI），每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in ORDINARY_WORK_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-highfreq-ordinary-work-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ ordinaryWorkTopicLabel() }} · {{ ordinaryWorkDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ ordinaryWorkTopicLabel() }} · {{ ordinaryWorkDifficultyLabel(selectedDifficulty!) }}</span>
