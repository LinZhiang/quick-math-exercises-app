...
import {
  useExtremumTest,
  type ExtremumResultRow,
} from '@/composables/math/useExtremumTest'
import {
  EXTREMUM_MODES,
  extremumDifficultyLabel,
  extremumTopicLabel,
  type ExtremumDifficulty,
  type ExtremumQuestion,
} from '@/utils/math/extremumPractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<ExtremumDifficulty | null>(null)
const test = reactive(useExtremumTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<ExtremumResultRow | null>(null)
...
function selectDifficulty(d: ExtremumDifficulty) {
...
function openResultDetail(row: ExtremumResultRow) {
  detailRow.value = row
...
function optionClass(row: ExtremumResultRow, idx: number) {
...
function passageHtml(q: ExtremumQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: ExtremumQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: ExtremumQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「其他运算·最值问题」：二次函数最值、最不利原则、和定最值。简单对齐真题 2 与示例；普通对齐真题 1、3；困难为更高阶变式。本地组卷，每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in EXTREMUM_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-other-extremum-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ extremumTopicLabel() }} · {{ extremumDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ extremumTopicLabel() }} · {{ extremumDifficultyLabel(selectedDifficulty!) }}</span>
