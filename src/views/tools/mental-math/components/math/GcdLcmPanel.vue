...
import {
  useGcdLcmTest,
  type GcdLcmResultRow,
} from '@/composables/math/useGcdLcmTest'
import {
  GCD_LCM_MODES,
  gcdLcmDifficultyLabel,
  gcdLcmTopicLabel,
  type GcdLcmDifficulty,
  type GcdLcmQuestion,
} from '@/utils/math/gcdLcmPractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<GcdLcmDifficulty | null>(null)
const test = reactive(useGcdLcmTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<GcdLcmResultRow | null>(null)
...
function selectDifficulty(d: GcdLcmDifficulty) {
...
function openResultDetail(row: GcdLcmResultRow) {
  detailRow.value = row
...
function optionClass(row: GcdLcmResultRow, idx: number) {
...
function passageHtml(q: GcdLcmQuestion | null | undefined) {
  if (!q?.passage) return ''
  return renderDataAnalysisMathHtml(q.passage).replace(/\n/g, '<br />')
}

function stemHtml(q: GcdLcmQuestion | null | undefined) {
  if (!q?.stem) return ''
  return renderDataAnalysisMathHtml(q.stem)
}

function explainHtml(q: GcdLcmQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「公因数与公倍数」：最大公因数/最小公倍数；普通含周期相遇（经典真题 3）；困难对齐经典真题 4（等差连乘末尾 0）。本地程序组卷（不调用
        AI），每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in GCD_LCM_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-skill-gcd-lcm-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ gcdLcmTopicLabel() }} · {{ gcdLcmDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ gcdLcmTopicLabel() }} · {{ gcdLcmDifficultyLabel(selectedDifficulty!) }}</span>
