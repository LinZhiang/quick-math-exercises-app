...
import {
  useColoringTest,
  type ColoringResultRow,
} from '@/composables/math/useColoringTest'
import {
  COLORING_MODES,
  coloringDifficultyLabel,
  coloringTopicLabel,
  type ColoringDifficulty,
  type ColoringQuestion,
} from '@/utils/math/coloringPractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<ColoringDifficulty | null>(null)
const test = reactive(useColoringTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<ColoringResultRow | null>(null)
...
function selectDifficulty(d: ColoringDifficulty) {
...
function openResultDetail(row: ColoringResultRow) {
  detailRow.value = row
...
function optionClass(row: ColoringResultRow, idx: number) {
...
function passageHtml(q: ColoringQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: ColoringQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: ColoringQuestion | null | undefined) {
...
>
      ...
      <div class="mode-grid">
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-highfreq-coloring-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ coloringTopicLabel() }} · {{ coloringDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ coloringTopicLabel() }} · {{ coloringDifficultyLabel(selectedDifficulty!) }}</span>
