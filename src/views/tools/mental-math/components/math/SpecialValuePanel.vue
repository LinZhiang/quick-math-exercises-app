...
import {
  useSpecialValueTest,
  type SpecValResultRow,
} from '@/composables/math/useSpecialValueTest'
import {
  SPEC_VAL_MODES,
  specValDifficultyLabel,
  specValTopicLabel,
  type SpecValDifficulty,
  type SpecValQuestion,
} from '@/utils/math/specialValuePractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<SpecValDifficulty | null>(null)
const test = reactive(useSpecialValueTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<SpecValResultRow | null>(null)
...
function selectDifficulty(d: SpecValDifficulty) {
...
function openResultDetail(row: SpecValResultRow) {
  detailRow.value = row
...
function optionClass(row: SpecValResultRow, idx: number) {
...
function passageHtml(q: SpecValQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: SpecValQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: SpecValQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「特值法」：设 1 / 设 100 / 设公倍数 / 设份数；困难为更难变式（每题题型不同）。本地程序组卷（不调用
        AI），每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in SPEC_VAL_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-skill-spec-val-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ specValTopicLabel() }} · {{ specValDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ specValTopicLabel() }} · {{ specValDifficultyLabel(selectedDifficulty!) }}</span>
