...
import {
  useChickenRabbitTest,
  type ChickenRabbitResultRow,
} from '@/composables/math/useChickenRabbitTest'
import {
  CHICKEN_RABBIT_MODES,
...
const selectedDifficulty = ref<ChickenRabbitDifficulty | null>(null)
const test = reactive(useChickenRabbitTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<ChickenRabbitResultRow | null>(null)
...
function selectDifficulty(d: ChickenRabbitDifficulty) {
...
function openResultDetail(row: ChickenRabbitResultRow) {
  detailRow.value = row
...
function optionClass(row: ChickenRabbitResultRow, idx: number) {
...
function passageHtml(q: ChickenRabbitQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: ChickenRabbitQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: ChickenRabbitQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「其他运算·鸡兔同笼问题」：假设法。全设为甲类，用（实际−假设）÷（乙属性−甲属性）求乙的数量。简单比经典真题更易；普通对齐经典真题（大中客车）；困难为更高阶变式。本地组卷，每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in CHICKEN_RABBIT_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-other-chicken-rabbit-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ chickenRabbitTopicLabel() }} · {{ chickenRabbitDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ chickenRabbitTopicLabel() }} · {{ chickenRabbitDifficultyLabel(selectedDifficulty!) }}</span>
