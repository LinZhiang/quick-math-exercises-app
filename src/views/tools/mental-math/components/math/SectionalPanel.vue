...
import {
  useSectionalTest,
  type SectionalResultRow,
} from '@/composables/math/useSectionalTest'
import {
  SECTIONAL_MODES,
...
const selectedDifficulty = ref<SectionalDifficulty | null>(null)
const test = reactive(useSectionalTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<SectionalResultRow | null>(null)
...
function selectDifficulty(d: SectionalDifficulty) {
...
function openResultDetail(row: SectionalResultRow) {
  detailRow.value = row
...
function optionClass(row: SectionalResultRow, idx: number) {
...
function passageHtml(q: SectionalQuestion | null | undefined) {
  if (!q?.passage) return ''
  return mathHtml(q.passage)
}

function stemHtml(q: SectionalQuestion | null | undefined) {
  if (!q?.stem) return ''
  return mathHtml(q.stem)
}

function explainHtml(q: SectionalQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「其他运算·分段问题」：不同区间不同计价；求最大用量时优先填满低价段。简单比经典真题更易；普通对齐经典真题（阶梯气价）；困难为更高阶变式。本地组卷，每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in SECTIONAL_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-other-sectional-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ sectionalTopicLabel() }} · {{ sectionalDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ sectionalTopicLabel() }} · {{ sectionalDifficultyLabel(selectedDifficulty!) }}</span>
