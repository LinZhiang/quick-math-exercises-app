...
import {
  ...
  type PrimeCompResultRow,
} from '@/composables/math/usePrimeCompositeTest'
import {
  PRIME_COMP_MODES,
...
const detailRow = ref<PrimeCompResultRow | null>(null)
...
function selectDifficulty(d: PrimeCompDifficulty) {
...
function openResultDetail(row: PrimeCompResultRow) {
  detailRow.value = row
...
function optionClass(row: PrimeCompResultRow, idx: number) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「质数与合数」：质数/合数判定、1 的特殊性、短除法质因数分解；困难对齐经典真题 2（三质数求和与两两积）。本地程序组卷（不调用
        AI），每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in PRIME_COMP_MODES"
          :key="m.id"
          type="button"
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ primeCompTopicLabel() }} · {{ primeCompDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ primeCompTopicLabel() }} · {{ primeCompDifficultyLabel(selectedDifficulty!) }}</span>
