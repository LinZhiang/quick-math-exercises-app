...
} from '@/composables/math/useCompetitionTest'
import {
  COMPETITION_MODES,
  competitionDifficultyLabel,
  competitionTopicLabel,
  type CompetitionDifficulty,
  type CompetitionQuestion,
} from '@/utils/math/competitionPractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<CompetitionDifficulty | null>(null)
const test = reactive(useCompetitionTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<CompetitionResultRow | null>(null)
...
function selectDifficulty(d: CompetitionDifficulty) {
...
function openResultDetail(row: CompetitionResultRow) {
  detailRow.value = row
...
function optionClass(row: CompetitionResultRow, idx: number) {
...
function passageHtml(q: CompetitionQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: CompetitionQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: CompetitionQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「其他运算·比赛问题」：淘汰赛（场次 n−1 / 排前四为 n）、单循环 C_n^2、双循环 A_n^2；握手题用连线图。简单比经典真题更易；普通对齐经典真题（五人握手）；困难为更高阶变式。本地组卷，每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in COMPETITION_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-other-competition-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ competitionTopicLabel() }} · {{ competitionDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ competitionTopicLabel() }} · {{ competitionDifficultyLabel(selectedDifficulty!) }}</span>
