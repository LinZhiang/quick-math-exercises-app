...
import {
  useSimilarTriangleTest,
  type SimilarTriangleResultRow,
} from '@/composables/math/useSimilarTriangleTest'
import {
  SIMILAR_TRIANGLE_MODES,
  similarTriangleDifficultyLabel,
  similarTriangleTopicLabel,
  type SimilarTriangleDifficulty,
  type SimilarTriangleQuestion,
} from '@/utils/math/similarTrianglePractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<SimilarTriangleDifficulty | null>(null)
const test = reactive(useSimilarTriangleTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<SimilarTriangleResultRow | null>(null)
...
function selectDifficulty(d: SimilarTriangleDifficulty) {
...
function openResultDetail(row: SimilarTriangleResultRow) {
  detailRow.value = row
...
function optionClass(row: SimilarTriangleResultRow, idx: number) {
...
function passageHtml(q: SimilarTriangleQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: SimilarTriangleQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: SimilarTriangleQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「三角形相似」：对应边成比例、面积比=相似比²；A 型平行截线、X 型沙漏、中位线；普通对齐经典真题 4（竹竿测树高）；困难为更高阶变式（每题题型不同）。本地程序组卷（不调用
        AI），每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in SIMILAR_TRIANGLE_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-highfreq-similar-triangle-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ similarTriangleTopicLabel() }} · {{ similarTriangleDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ similarTriangleTopicLabel() }} · {{ similarTriangleDifficultyLabel(selectedDifficulty!) }}</span>
