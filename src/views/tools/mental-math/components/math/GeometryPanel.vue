...
import { useGeometryTest, type GeometryResultRow } from '@/composables/math/useGeometryTest'
import {
  GEOMETRY_MODES,
  geometryDifficultyLabel,
  geometryTopicLabel,
...
const selectedDifficulty = ref<GeometryDifficulty | null>(null)
const test = reactive(useGeometryTest(selectedDifficulty))
...
function selectDifficulty(d: GeometryDifficulty) {
...
function openResultDetail(row: GeometryResultRow) {
  detailRow.value = row
...
function optionClass(row: GeometryResultRow, idx: number) {
...
function passageHtml(q: GeometryQuestion | null | undefined) {
  if (!q?.passage) return ''
  return renderDataAnalysisMathHtml(q.passage).replace(/\n/g, '<br />')
}

function stemHtml(q: GeometryQuestion | null | undefined) {
  if (!q?.stem) return ''
  return renderDataAnalysisMathHtml(q.stem)
}

function explainHtml(q: GeometryQuestion | null | undefined) {
...
>
      ...
      <div class="mode-grid">
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-highfreq-geometry-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ geometryTopicLabel() }} · {{ geometryDifficultyLabel(selectedDifficulty) }}。每轮
        {{ test.questionCount }} 题 · 豆包出题 + 几何图。正计时，提交后暂停看答案。
      </p>
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ geometryTopicLabel() }} · {{ geometryDifficultyLabel(selectedDifficulty!) }}</span>
      ...
      <GeometryFigureView v-if="test.currentQuestion?.figure" :figure="test.currentQuestion.figure" />
    ...
    <template v-else-if="test.phase === 'summary'">
      ...
      <p class="chinese-summary__hint">点击题目可查看图形、题干、选项与解析</p>
      ...
      >
        <div v-if="detailRow" class="da-result-detail">
          ...
          <GeometryFigureView
            v-if="detailRow.question.figure"
            :figure="detailRow.question.figure"
          />
