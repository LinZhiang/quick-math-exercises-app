...
import {
  useClockTest,
  type ClockResultRow,
} from '@/composables/math/useClockTest'
import {
  CLOCK_MODES,
  clockDifficultyLabel,
  clockTopicLabel,
  type ClockDifficulty,
  type ClockQuestion,
} from '@/utils/math/clockPractice'
...
const selectedDifficulty = ref<ClockDifficulty | null>(null)
const test = reactive(useClockTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<ClockResultRow | null>(null)
...
function selectDifficulty(d: ClockDifficulty) {
...
function openResultDetail(row: ClockResultRow) {
  detailRow.value = row
...
function optionClass(row: ClockResultRow, idx: number) {
...
function passageHtml(q: ClockQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: ClockQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: ClockQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「其他运算·时钟问题」：钟面追及（6°/0.5°/5.5°）、夹角公式、坏钟比例。简单对齐真题 1；普通对齐真题 2；困难为更高阶变式。本地组卷，每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in CLOCK_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-other-clock-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ clockTopicLabel() }} · {{ clockDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ clockTopicLabel() }} · {{ clockDifficultyLabel(selectedDifficulty!) }}</span>
      ...
      <div v-if="test.submitted && test.currentQuestion" class="chinese-quiz__feedback">
        ...
        />
        <ClockFaceDiagram
          v-if="test.currentQuestion?.clockDiagram"
          :hour-deg="test.currentQuestion.clockDiagram.hourDeg"
          :minute-deg="test.currentQuestion.clockDiagram.minuteDeg"
          :time-label="test.currentQuestion.clockDiagram.timeLabel"
          :caption="test.currentQuestion.clockDiagram.caption"
          :show-arc="test.currentQuestion.clockDiagram.showArc !== false"
        />
        <div
      ...
      >
        <div v-if="detailRow" class="da-result-detail">
          ...
          <section v-if="detailRow.question.method || detailRow.question.explanation">
            ...
            />
            <ClockFaceDiagram
              v-if="detailRow.question.clockDiagram"
              :hour-deg="detailRow.question.clockDiagram.hourDeg"
              :minute-deg="detailRow.question.clockDiagram.minuteDeg"
              :time-label="detailRow.question.clockDiagram.timeLabel"
              :caption="detailRow.question.clockDiagram.caption"
              :show-arc="detailRow.question.clockDiagram.showArc !== false"
            />
          </section>
        </div>
        <template #footer>
