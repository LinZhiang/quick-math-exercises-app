...
import {
  useProbabilityTest,
  type ProbabilityResultRow,
} from '@/composables/math/useProbabilityTest'
import {
  PROBABILITY_MODES,
  probabilityDifficultyLabel,
  probabilityTopicLabel,
  type ProbabilityDifficulty,
...
const selectedDifficulty = ref<ProbabilityDifficulty | null>(null)
const test = reactive(useProbabilityTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<ProbabilityResultRow | null>(null)
...
function selectDifficulty(d: ProbabilityDifficulty) {
...
function openResultDetail(row: ProbabilityResultRow) {
  detailRow.value = row
...
function optionClass(row: ProbabilityResultRow, idx: number) {
...
function passageHtml(q: ProbabilityQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: ProbabilityQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: ProbabilityQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「排列组合·概率问题」：古典概率、分类分步、几何概率。简单对齐真题 1；普通对齐真题 2；困难对齐真题 3（豆包改写）。每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in PROBABILITY_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-highfreq-probability-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ probabilityTopicLabel() }} · {{ probabilityDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ probabilityTopicLabel() }} · {{ probabilityDifficultyLabel(selectedDifficulty!) }}</span>
