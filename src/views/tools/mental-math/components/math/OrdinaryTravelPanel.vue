...
import {
  useOrdinaryTravelTest,
  type OrdinaryTravelResultRow,
} from '@/composables/math/useOrdinaryTravelTest'
import {
  ORDINARY_TRAVEL_MODES,
  ordinaryTravelDifficultyLabel,
  ordinaryTravelTopicLabel,
  type OrdinaryTravelDifficulty,
  type OrdinaryTravelQuestion,
} from '@/utils/math/ordinaryTravelPractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<OrdinaryTravelDifficulty | null>(null)
const test = reactive(useOrdinaryTravelTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<OrdinaryTravelResultRow | null>(null)
...
function selectDifficulty(d: OrdinaryTravelDifficulty) {
...
function openResultDetail(row: OrdinaryTravelResultRow) {
  detailRow.value = row
...
function optionClass(row: OrdinaryTravelResultRow, idx: number) {
...
function passageHtml(q: OrdinaryTravelQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: OrdinaryTravelQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: OrdinaryTravelQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「普通行程问题」：核心公式 s=v×t；时间/速度/路程一定时的正反比；一般平均速度与等距特值 2v1v2/(v1+v2)。普通对齐经典真题 1～3。本地程序组卷（不调用
        AI），每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in ORDINARY_TRAVEL_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-highfreq-ordinary-travel-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ ordinaryTravelTopicLabel() }} · {{ ordinaryTravelDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ ordinaryTravelTopicLabel() }} · {{ ordinaryTravelDifficultyLabel(selectedDifficulty!) }}</span>
