...
import {
  useCrossMethodTest,
  type CrossMethodResultRow,
} from '@/composables/math/useCrossMethodTest'
import {
  CROSS_METHOD_MODES,
  crossMethodDifficultyLabel,
  crossMethodTopicLabel,
  type CrossMethodDifficulty,
  type CrossMethodQuestion,
} from '@/utils/math/crossMethodPractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<CrossMethodDifficulty | null>(null)
const test = reactive(useCrossMethodTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<CrossMethodResultRow | null>(null)
...
function selectDifficulty(d: CrossMethodDifficulty) {
...
function openResultDetail(row: CrossMethodResultRow) {
  detailRow.value = row
...
function optionClass(row: CrossMethodResultRow, idx: number) {
...
function passageHtml(q: CrossMethodQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: CrossMethodQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: CrossMethodQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「十字交叉法」：混合比例速算；普通对齐经典真题（先算混合率再交叉）；困难为更高阶变式。本地程序组卷（不调用
        AI），每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in CROSS_METHOD_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-skill-cross-method-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ crossMethodTopicLabel() }} · {{ crossMethodDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ crossMethodTopicLabel() }} · {{ crossMethodDifficultyLabel(selectedDifficulty!) }}</span>
