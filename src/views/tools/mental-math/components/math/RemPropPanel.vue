...
import {
  useRemPropTest,
  type RemPropResultRow,
} from '@/composables/math/useRemPropTest'
import {
  REM_PROP_MODES,
  remPropDifficultyLabel,
  remPropTopicLabel,
  type RemPropDifficulty,
  type RemPropQuestion,
} from '@/utils/math/remPropPractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<RemPropDifficulty | null>(null)
const test = reactive(useRemPropTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<RemPropResultRow | null>(null)
...
function selectDifficulty(d: RemPropDifficulty) {
...
function openResultDetail(row: RemPropResultRow) {
  detailRow.value = row
...
function optionClass(row: RemPropResultRow, idx: number) {
...
function passageHtml(q: RemPropQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: RemPropQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: RemPropQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「余数及其性质」：同余性质；普通对齐经典真题（赛点/分组）；困难为更难变式（每题题型不同）。本地程序组卷（不调用
        AI），每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in REM_PROP_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-skill-rem-prop-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ remPropTopicLabel() }} · {{ remPropDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ remPropTopicLabel() }} · {{ remPropDifficultyLabel(selectedDifficulty!) }}</span>
