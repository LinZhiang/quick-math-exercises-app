...
import {
  useProfitCalcTest,
  type ProfitCalcResultRow,
} from '@/composables/math/useProfitCalcTest'
import {
  PROFIT_CALC_MODES,
  profitCalcDifficultyLabel,
  profitCalcTopicLabel,
  type ProfitCalcDifficulty,
  type ProfitCalcQuestion,
} from '@/utils/math/profitCalcPractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<ProfitCalcDifficulty | null>(null)
const test = reactive(useProfitCalcTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<ProfitCalcResultRow | null>(null)
...
function selectDifficulty(d: ProfitCalcDifficulty) {
...
function openResultDetail(row: ProfitCalcResultRow) {
  detailRow.value = row
...
function optionClass(row: ProfitCalcResultRow, idx: number) {
...
function passageHtml(q: ProfitCalcQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: ProfitCalcQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: ProfitCalcQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「利润计算」：利润=售价(收入)−成本(支出)。对齐经典真题 1（囤货涨价+仓储+损耗求天数/利润）；不考利润率。仅简单/困难两档。本地程序组卷（不调用
        AI），每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in PROFIT_CALC_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-highfreq-profit-calc-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ profitCalcTopicLabel() }} · {{ profitCalcDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ profitCalcTopicLabel() }} · {{ profitCalcDifficultyLabel(selectedDifficulty!) }}</span>
