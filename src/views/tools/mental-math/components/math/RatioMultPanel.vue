...
import {
  useRatioMultTest,
  type RatioMultResultRow,
} from '@/composables/math/useRatioMultTest'
import {
  RATIO_MULT_MODES,
  ratioMultDifficultyLabel,
  ratioMultTopicLabel,
  type RatioMultDifficulty,
  type RatioMultQuestion,
} from '@/utils/math/ratioMultPractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<RatioMultDifficulty | null>(null)
const test = reactive(useRatioMultTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<RatioMultResultRow | null>(null)
...
function selectDifficulty(d: RatioMultDifficulty) {
...
function openResultDetail(row: RatioMultResultRow) {
  detailRow.value = row
...
function optionClass(row: RatioMultResultRow, idx: number) {
...
function passageHtml(q: RatioMultQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: RatioMultQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: RatioMultQuestion | null | undefined) {
...
>
      ...
      <div class="mode-grid">
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-skill-ratio-mult-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ ratioMultTopicLabel() }} · {{ ratioMultDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ ratioMultTopicLabel() }} · {{ ratioMultDifficultyLabel(selectedDifficulty!) }}</span>
