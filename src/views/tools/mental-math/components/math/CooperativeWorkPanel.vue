...
import {
  useCooperativeWorkTest,
  type CooperativeWorkResultRow,
} from '@/composables/math/useCooperativeWorkTest'
import {
  COOPERATIVE_WORK_MODES,
  cooperativeWorkDifficultyLabel,
  cooperativeWorkTopicLabel,
  type CooperativeWorkDifficulty,
  type CooperativeWorkQuestion,
} from '@/utils/math/cooperativeWorkPractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<CooperativeWorkDifficulty | null>(null)
const test = reactive(useCooperativeWorkTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<CooperativeWorkResultRow | null>(null)
...
function selectDifficulty(d: CooperativeWorkDifficulty) {
...
function openResultDetail(row: CooperativeWorkResultRow) {
  detailRow.value = row
...
function optionClass(row: CooperativeWorkResultRow, idx: number) {
...
function passageHtml(q: CooperativeWorkQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: CooperativeWorkQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: CooperativeWorkQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「合作完工问题」：效率和=各效率之和；总量=各人工作量之和。简单对齐或略低于经典真题 2；仅简单/困难两档。本地程序组卷（不调用
        AI），每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in COOPERATIVE_WORK_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-highfreq-cooperative-work-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ cooperativeWorkTopicLabel() }} · {{ cooperativeWorkDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ cooperativeWorkTopicLabel() }} · {{ cooperativeWorkDifficultyLabel(selectedDifficulty!) }}</span>
