...
import {
  useAgeTest,
  type AgeResultRow,
} from '@/composables/math/useAgeTest'
import {
  AGE_MODES,
  ageDifficultyLabel,
  ageTopicLabel,
  type AgeDifficulty,
  type AgeQuestion,
} from '@/utils/math/agePractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<AgeDifficulty | null>(null)
const test = reactive(useAgeTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<AgeResultRow | null>(null)
...
function selectDifficulty(d: AgeDifficulty) {
...
function openResultDetail(row: AgeResultRow) {
  detailRow.value = row
...
function optionClass(row: AgeResultRow, idx: number) {
...
function passageHtml(q: AgeQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: AgeQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: AgeQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「其他运算·年龄问题」：年龄差不变、同期同增、出生年、经典数轴三段。简单比经典真题更直接；普通对齐经典真题；困难为更高阶变式。本地组卷，每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in AGE_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-other-age-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ ageTopicLabel() }} · {{ ageDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ ageTopicLabel() }} · {{ ageDifficultyLabel(selectedDifficulty!) }}</span>
