...
import {
  useDivisibilityJudgeTest,
  type DivJudgeResultRow,
} from '@/composables/math/useDivisibilityJudgeTest'
import {
  DIV_JUDGE_MODES,
  divJudgeDifficultyLabel,
  divJudgeTopicLabel,
  type DivJudgeDifficulty,
  type DivJudgeQuestion,
} from '@/utils/math/divisibilityJudgePractice'
import PracticeCompletionStat from '@/views/tools/mental-math/components/shared/PracticeCompletionStat.vue'

const selectedDifficulty = ref<DivJudgeDifficulty | null>(null)
const test = reactive(useDivisibilityJudgeTest(selectedDifficulty))
const regenerating = ref(false)
const detailRow = ref<DivJudgeResultRow | null>(null)
...
function selectDifficulty(d: DivJudgeDifficulty) {
...
function openResultDetail(row: DivJudgeResultRow) {
  detailRow.value = row
...
function optionClass(row: DivJudgeResultRow, idx: number) {
...
function passageHtml(q: DivJudgeQuestion | null | undefined) {
  if (!q?.passage) return ''
  return esc(q.passage)
}

function stemHtml(q: DivJudgeQuestion | null | undefined) {
  if (!q?.stem) return ''
  return esc(q.stem)
}

function explainHtml(q: DivJudgeQuestion | null | undefined) {
...
>
    <template v-if="!selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        考点「整除的判定」：能被 3/9/7/11/6/14 等整除的判定口诀，以及「谁能被整除 / 能被谁整除」。本地程序组卷（不调用
        AI），每轮 {{ test.questionCount }} 题四选一。正计时，提交后暂停看答案。
      </p>
      <div class="mode-grid">
        <button
          v-for="m in DIV_JUDGE_MODES"
          :key="m.id"
          type="button"
        ...
        >
          <h3 class="mode-card__title">
            {{ m.label }}
            <PracticeCompletionStat :mode-id="`op-skill-div-judge-${m.id}`" />
          </h3>
    ...
    <template v-else-if="selectedDifficulty && (test.phase === 'idle' || test.phase === 'loading')">
      <p class="mode-section__hint">
        当前：{{ divJudgeTopicLabel() }} · {{ divJudgeDifficultyLabel(selectedDifficulty) }}。每轮
    ...
    <template v-else-if="test.phase === 'running'">
      <div class="chinese-quiz__top">
        ...
        <span>{{ divJudgeTopicLabel() }} · {{ divJudgeDifficultyLabel(selectedDifficulty!) }}</span>
