...
import {
  ...
  ensureFrontendLearningNodeLoaded,
  ensureFrontendLearningSubtreeLoaded,
  loadFrontendLearningItem,
  loadFrontendLearningTree,
  moveFrontendItem,
  moveFrontendNode,
  pullFrontendLearningFromCloud,
  reloadFrontendLearningDir,
  renameFrontendNode,
...
const folderTree = ref<FrontendTreeNode[]>([])
...
async function toggle(id: string, expandable: boolean) {
  ...
  if (!willOpen) return
  const node = findFrontendNode(tree.value, id)?.node
  if (node?.loaded === false) {
    loadingNodeId.value = id
    try {
      tree.value = await ensureFrontendLearningNodeLoaded(id, true)
    } catch (e) {
...
async function startFolderQuiz(nodeId: string, name: string) {
  const full = await loadFrontendLearningTree()
  const pos = findFrontendNode(full, nodeId)
  if (!pos) return
  ...
  try {
    await withBusy('正在读取范围内讲义…', async () => {
      ...
      const learningPath = frontendNodePathNames(full, nodeId)
...
function canDownloadRow(row: FrontendTreeRow) {
  if (row.locked && !isAdmin.value) return false
  if (row.kind === 'entry' && !row.entry.ready && !isAdmin.value) return false
  return true
}

async function downloadFrontendEntry(entry: FrontendTreeEntry) {
  if (!isAdmin.value && (!entry.ready || entry.private)) {
    throw new Error(entry.ready ? EXPORT_DENIED : '该小节即将开放')
  }
  const item = await loadFrontendLearningItem(entry.id)
  await exportHandoutDocx(item.title, frontendContentToHtml(item.content))
}

async function downloadFrontendFolder(nodeId: string, name: string) {
  const node = await ensureFrontendLearningSubtreeLoaded(nodeId)
  ...
  for (const ref of refs) {
    try {
      const item = await loadFrontendLearningItem(ref.id)
      packed.push({
        path: ref.path,
        title: item.title || ref.title,
        html: frontendContentToHtml(item.content),
      })
    } catch {
      /* 无权限或缺失的讲义不写入文档 */
    }
...
async function onDownloadRow(row: FrontendTreeRow) {
  ...
  try {
    await withBusy('正在导出 Word…', async () => {
      if (row.kind === 'entry') await downloadFrontendEntry(row.entry)
      else await downloadFrontendFolder(row.id, row.name)
      adminOpenId.value = ''
    })
...
async function syncCloudIfLocalAdmin(force = false) {
  ...
  try {
    return await pullFrontendLearningFromCloud()
  } catch (e) {
    cloudPullOnce = false
    throw e
  }
...
function catalogEntryCount(nodes: FrontendTreeNode[]): number {
...
async function load(force = false) {
  ...
  try {
    const next = await loadFrontendLearningTree(force)
    if (gen !== catalogLoadGen) return
    tree.value = next
...
async function forceRefreshCatalog() {
  ...
  try {
    ...
    if (gen !== catalogLoadGen) return
    clearFrontendLearningCache()
    await wipeHandoutDiskCacheScope('frontend')
    if (gen !== catalogLoadGen) return
    let next: FrontendTreeNode[]
    try {
      next = await loadFrontendLearningTree(true)
    } catch {
      next = await reloadFrontendLearningDir(Object.keys(keep).filter((id) => keep[id]))
    }
    if (gen !== catalogLoadGen) return
    tree.value = next
...
watch(mapOpen, async (on) => {
  if (!on) return
  try {
    folderTree.value = await loadFrontendLearningTree()
  } catch {
...
<template>
  <section class="computer-page" :class="{ 'is-quiz': Boolean(quizItem) }">
    ...
    <FrontendMoveDialog
      ...
      @confirm="confirmMove"
    />
    <FrontendCategoryMapDialog v-model="mapOpen" :tree="folderTree.length ? folderTree : tree" />
