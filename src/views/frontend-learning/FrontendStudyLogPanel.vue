...
import {
  clearFrontendStudyLogs,
  frontendQuizResultText,
  frontendStudyDaySummary,
  frontendStudyLogTick,
  filterFrontendStudyLogs,
  formatFrontendStudyDateTitle,
  formatFrontendStudyTime,
  groupFrontendStudyLogsByDate,
  listFrontendStudyLogs,
...
const allLogs = computed(() => {
  ...
  return listFrontendStudyLogs()
})

const quizStats = computed(() => summarizeFrontendQuizLogs())

const days = computed(() =>
  groupFrontendStudyLogsByDate(
    filterFrontendStudyLogs({ dateKey: filterDate.value || undefined }),
  ),
)
  ...
  <section class="cb-log">
    ...
    <div v-if="days.length" class="cb-log__days">
      <article v-for="day in days" :key="day.dateKey" class="cb-log__day">
        <h4 class="cb-log__day-title">{{ formatFrontendStudyDateTitle(day.dateKey, todayKey) }}</h4>
        <p class="cb-log__day-sum">{{ frontendStudyDaySummary(day) }}</p>
        ...
        <section class="cb-log__block">
          ...
          <ul v-if="day.views.length">
            <li v-for="row in day.views" :key="row.id">
              ...
              <span class="cb-log__time">最近打开 {{ formatFrontendStudyTime(row.at) }}</span>
            </li>
          </ul>
        ...
        <section class="cb-log__block">
          ...
          <ul v-if="day.quizzes.length">
            <li v-for="row in day.quizzes" :key="row.id">
              <span class="cb-log__name">{{ row.itemTitle }}</span>
              <span v-if="row.pathLabel" class="cb-log__path">路径 {{ row.pathLabel }}</span>
              <span class="cb-log__result">{{ frontendQuizResultText(row) }}</span>
              <span class="cb-log__time">完成于 {{ formatFrontendStudyTime(row.at) }}</span>
            </li>
          </ul>
