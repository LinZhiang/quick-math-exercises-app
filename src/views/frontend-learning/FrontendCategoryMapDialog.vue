...
function entrySource(entry: FrontendTreeEntry): MapSource {
  return {
    id: `entry:${entry.id}`,
    name: entry.title,
    expandable: false,
...
function nodeSource(node: FrontendTreeNode): MapSource {
  return {
    id: node.id,
    name: node.name,
...
function layoutMindmap(tree: FrontendTreeNode[], openMap: Record<string, boolean>) {
