...
export type ChineseHistoryCommonSenseQuestionType = 'general'

export const HISTORY_COMMON_SENSE_QUESTION_COUNT = 15

export type HistoryCommonSenseQuestion = {
  id: string
  questionType: ChineseHistoryCommonSenseQuestionType
  /** 知识点，如「辛亥革命」「贞观之治」 */
  term: string
  stem: string
  options: string[]
...
export function historyCommonSenseQuestionTypeLabel(
  _type: ChineseHistoryCommonSenseQuestionType,
): string {
  return '历史常识'
}

export function getHistoryCommonSenseQuestionFingerprint(input: {
  questionType: ChineseHistoryCommonSenseQuestionType
  term: string
  stem: string
  options: string[]
...
export function buildHistoryCommonSenseQuestionFromMcq(input: {
  questionType: ChineseHistoryCommonSenseQuestionType
  term: string
  stem: string
  correct: string
  ...
  explanation?: string
  seq: number
}): HistoryCommonSenseQuestion | null {
  ...
  const fingerprint = getHistoryCommonSenseQuestionFingerprint({
  ...
  const q: HistoryCommonSenseQuestion = {
    id: `hist-sense-${input.seq}-${Date.now()}`,
    questionType: input.questionType,
    term,
    stem,
    options,
...
export function parseHistoryCommonSenseMcqAiObject(item: unknown): {
  questionType: ChineseHistoryCommonSenseQuestionType
  term: string
  stem: string
  correct: string
