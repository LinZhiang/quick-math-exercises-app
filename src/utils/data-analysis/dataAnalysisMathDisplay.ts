...
function normalizeLatex(s: string): string {
  ...
  out = out.replace(/\\oplus/g, '⊕').replace(/\\otimes/g, '⊗').replace(/\\cdot/g, '·')
  out = out.replace(/\\wedge/g, '∧').replace(/\\vee/g, '∨').replace(/\\neg/g, '¬')
  out = out.replace(/\\%/g, '%')
  for (let n = 0; n < 8; n++) {
    const next = out
      .replace(/\\dfrac\{([^{}]+)\}\{([^{}]+)\}/g, '($1)/($2)')
...
function extractOverlines(s: string, slots: { inner: string }[]): string {
  return s.replace(/\\(?:overline|bar)\{([^{}]+)\}/g, (_m, inner: string) => {
    const id = slots.length
    slots.push({ inner: String(inner) })
    return `\uE700${id}\uE701`
  })
}

function expandOverlineTokens(html: string, slots: { inner: string }[]): string {
  return html.replace(/\uE700(\d+)\uE701/g, (_m, id: string) => {
    const slot = slots[Number(id)]
    if (!slot) return ''
    return `<span class="da-math-over">${escapeHtmlText(slot.inner)}</span>`
  })
}
...
export function renderDataAnalysisMathHtml(text: string): string {
  let s = normalizeLatex(text)
  const overs: { inner: string }[] = []
  s = extractOverlines(s, overs)
  ...
  html = expandOverlineTokens(html, overs)
