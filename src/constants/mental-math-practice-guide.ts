...
export const MENTAL_MATH_GUIDE_GROUPS: MentalMathGuideGroup[] = [
  ...
  },
  {
    ...
    articles: [
      ...
      {
        id: 'arithmetic-addsub',
        title: '加减 · 简单/复杂',
        category: 'arithmetic',
        practiceModeId: 'addsub-easy',
        blocks: [
          {
            type: 'p',
            text: '专练加减：简单题在 12 以内（可含负数）；复杂题为 11～99（可含负数，且个位不为 0）。计时与计分分别对齐「简单模式 / 高难模式」。干扰项参考干扰型：运算符错位、结果符号错位。',
          },
          {
            type: 'tip',
            text: '先定符号再算绝对值；复杂题注意进退位，别被「差一点」的选项带走。',
          },
        ],
      },
      {
        id: 'arithmetic-muldiv',
        title: '乘除 · 简单/复杂',
        category: 'arithmetic',
        practiceModeId: 'muldiv-easy',
        blocks: [
          {
            type: 'p',
            text: '专练乘除：简单题为个位相乘（可含负数），除法商为个位数；复杂题以 6～99 相乘为主（可含负数）。计时与计分对齐「简单模式 / 高难模式」。干扰含符号错位、邻档乘数（如 4×9 混入 4×8=32）。',
          },
          {
            type: 'tip',
            text: '乘法先定正负再口算；看到「差不多」的积，核对乘数是否邻档。',
          },
        ],
      },
      {
  ...
  },
  {
    ...
    articles: [
      ...
      },
      {
        ...
        title: '特殊分数值',
        category: 'fraction',
        practiceModeId: 'fraction-special-easy',
        blocks: [
          {
            type: 'p',
            text: '只考一张对照表里的单位分数与百分数互化：看见 1/n 写百分数，或看见百分数写 1/n。不考表外数字。',
          },
          {
            type: 'h3',
            text: '怎么练',
  ...
  },
  {
          ...
          },
          {
            type: 'ul',
            items: [
              '1/2=50%，1/3=33.3%，1/4=25%，1/5=20%，1/6=16.7%，1/7=14.3%，1/8=12.5%，1/9=11.1%，1/10=10%',
              '1/11=9.1%，1/12=8.3%，1/13=7.7%，1/14=7.1%，1/15=6.7%，1/16=6.3%，1/17=5.9%，1/18=5.6%，1/19=5.3%',
            ],
          },
          {
            type: 'tip',
            text: '小数百分数按表背（如 33.3%、16.7%），不要自己临时除出来改写法。',
          },
        ],
      },
    ],
    ...
    articles: [
      {
        ...
        blocks: [
          {
            type: 'p',
            text: '快判本地题：给出四个汉字，选出共同规律。覆盖笔画数、部件包含、封闭区域个数、文字结构、对称、连通块、封闭/开放等常见考法。不含笔画交叉数。仅普通题，无加深识记。',
          },
          {
            type: 'h3',
            text: '怎么练',
  ...
  },
  {
    ...
    articles: [
      {
        id: 'schulte-overview',
        title: '怎么练舒尔特（成语/词语/古诗词/生活常识）',
        category: 'schulte',
        practiceModeId: 'schulte-easy',
        blocks: [
          {
            type: 'p',
            text: '快判本地题：开局先出示内容（不计时），再渐显舒尔特方格并按字序点选。成语均为四字、词语均为二字；古诗词素材取自识记模块·诗词模块正式篇目；生活常识考点对齐快判「生活常识」题库（一问一答改写为陈述句）。',
          },
          {
            type: 'h3',
            text: '成语 / 词语 识记',
          },
          {
            type: 'ul',
            items: [
              '出示时长：成语 1.8 秒，词语 1 秒。',
              '简单题：28 秒，4 行 × 4 列，对 +15 / 错 −30。',
              '普通题：33 秒，6 行 × 4 列，对 +15 / 错 −30（计分同简单）。',
              '复杂题：40 秒，7 行 × 5 列，对 +23 / 错 −45。',
              '题库 250 成语 + 250 词语；侧重公考/事业编常考易错成语与易混近义词，释义点明易错点。',
              '干扰格优先抽取形近、易混的规范汉字（不含错字）。',
              '按词条去重：优先未出，出完重置再循环。',
            ],
          },
          {
          ...
          },
          {
            type: 'ul',
            items: [
              '出示形如「会当凌绝顶，一览众山小——杜甫」；时长按汉字数 ×0.35 秒（含作者名）。',
              '点选顺序为诗句汉字 + 作者名汉字（标点不入格）；全格字互不重复。',
              '识记字数：简单 ≤13 字；普通 ≤17 字；高难 ≥13 字。',
              '按知识点去重：优先未出过的诗句，该档出完后自动重置再循环。',
              '考点仅限识记模块·诗词模块正式篇目与原注释，不含「阶段概述」等非诗句材料。',
              '简单题：1 分 35 秒，4 行 × 4 列，对 +23 / 错 −45。',
              '普通题：1 分 57 秒，6 行 × 4 列，对 +23 / 错 −45（计分同简单）。',
              '高难题：2 分 18 秒，7 行 × 5 列，对 +30 / 错 −60。',
              '答完展示诗词模块原注释。',
            ],
          },
          {
          ...
          },
          {
            type: 'ul',
            items: [
              '原快判「生活常识」一问一答改写为陈述句后出示；时长按汉字数 ×0.35 秒。',
              '识记字数：简单 ≤13 字；普通 ≤17 字；复杂 ≥13 字（不合规陈述会改写或剔除）。',
              '格子与计分规则与「古诗词」三档一致；内容难度与原生活常识简单 / 普通 / 复杂一一对应。',
              '按知识点去重：同一题干+答案只算一题，优先未出，出完重置再循环。',
              '简单题：1 分 40 秒，4 行 × 4 列，对 +23 / 错 −45。',
              '普通题：2 分 1 秒，6 行 × 4 列，对 +23 / 错 −45（计分同简单）。',
              '复杂题：2 分 27 秒，7 行 × 5 列，对 +30 / 错 −60。',
              '答完展示原释义。',
            ],
          },
          {
            type: 'tip',
            text: '先把出示内容记牢，格子出现后按字序逐个点选；点错即停表看释义。',
          },
        ],
      },
    ],
