...
import { isAiChatConfigured, requestGeographyCommonSenseMcqs, DEEPSEEK_NOT_CONFIGURED_HINT } from '@/services/deepseek'
import {
  appendGeneratedTerms,
  listRecentGeneratedTerms,
} from '@/utils/chinese/chineseGeneratedHistory'
import { upsertChineseGeographyCommonSenseWrong } from '@/utils/chinese/chineseGeographyCommonSenseStorage'
...
export function useChineseGeographyCommonSenseTest() {
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      incrementPracticeCompletion('chinese-geography-common-sense', {
