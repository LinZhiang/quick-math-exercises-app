...
import { upsertChineseClassicalChineseWrong } from '@/utils/chinese/chineseClassicalChineseStorage'
...
import {
  ...
  classicalChineseQuestionTypeLabel,
...
export type ChineseClassicalChinesePhase = 'idle' | 'loading' | 'running' | 'summary'

export type ChineseClassicalChineseResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: ClassicalChineseQuestion
...
export function useChineseClassicalChineseTest() {
  const phase = ref<ChineseClassicalChinesePhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<ClassicalChineseQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const paperSource = ref<ChinesePaperSource>(null)
  const results = ref<ChineseClassicalChineseResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : CLASSICAL_CHINESE_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    try {
      const generated = await requestClassicalChineseMcqs({
        count: CLASSICAL_CHINESE_QUESTION_COUNT,
  ...
  function startQuiz(initialQuestions?: ClassicalChineseQuestion[], opts?: { keyReview?: ChineseKeyReviewMeta }) {
  ...
  async function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: classicalChineseQuestionTypeLabel(q.questionType),
      title: q.term,
      correct,
      question: q,
