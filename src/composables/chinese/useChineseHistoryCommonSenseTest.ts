...
import { isAiChatConfigured, requestHistoryCommonSenseMcqs, DEEPSEEK_NOT_CONFIGURED_HINT } from '@/services/deepseek'
import {
  appendGeneratedTerms,
  listRecentGeneratedTerms,
} from '@/utils/chinese/chineseGeneratedHistory'
import { upsertChineseHistoryCommonSenseWrong } from '@/utils/chinese/chineseHistoryCommonSenseStorage'
...
} from '@/utils/chinese/chineseKeyReviewSession'
import { noteWrongOrReplaceKeyReviewVariant } from '@/utils/chinese/chineseKeyReviewWrongReplace'
import { playMentalMathStartSound } from '@/utils/math/mentalMathSounds'
import {
  HISTORY_COMMON_SENSE_QUESTION_COUNT,
  historyCommonSenseQuestionTypeLabel,
...
export type ChineseHistoryCommonSensePhase = 'idle' | 'loading' | 'running' | 'summary'

export type ChineseHistoryCommonSenseResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: HistoryCommonSenseQuestion
...
export function useChineseHistoryCommonSenseTest() {
  const phase = ref<ChineseHistoryCommonSensePhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<HistoryCommonSenseQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const paperSource = ref<ChinesePaperSource>(null)
  const results = ref<ChineseHistoryCommonSenseResultRow[]>([])
  ...
  const wrongGate = createChineseWrongBookGate(upsertChineseHistoryCommonSenseWrong)
  const carelessMarked = ref(false)

  const currentQuestion = computed(() => questions.value[currentIndex.value] ?? null)
  const correctCount = computed(() => results.value.filter((r) => r.correct).length)
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : HISTORY_COMMON_SENSE_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    try {
      const generated = await requestHistoryCommonSenseMcqs({
        count: HISTORY_COMMON_SENSE_QUESTION_COUNT,
        avoidTerms: listRecentGeneratedTerms('history-common-sense'),
        onProgress: (msg) => {
  ...
  function startQuiz(initialQuestions?: HistoryCommonSenseQuestion[], opts?: { keyReview?: ChineseKeyReviewMeta }) {
  ...
  async function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: historyCommonSenseQuestionTypeLabel(q.questionType),
      title: q.term,
      correct,
      question: q,
