...
import { isAiChatConfigured, requestIdiomRecognitionMcqs, DEEPSEEK_NOT_CONFIGURED_HINT } from '@/services/deepseek'
import {
  appendGeneratedTerms,
  listRecentGeneratedTerms,
} from '@/utils/chinese/chineseGeneratedHistory'
import { upsertChineseWrong } from '@/utils/chinese/chineseIdiomStorage'
...
export type ChineseIdiomResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: IdiomRecognitionQuestion
...
export function useChineseIdiomTest() {
  const phase = ref<ChineseIdiomPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<IdiomRecognitionQuestion[]>([])
  ...
  const wrongGate = createChineseWrongBookGate(upsertChineseWrong)
  const carelessMarked = ref(false)

  const currentQuestion = computed(() => questions.value[currentIndex.value] ?? null)
  const correctCount = computed(() => results.value.filter((r) => r.correct).length)
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : IDIOM_RECOGNITION_QUESTION_COUNT,
  )
  ...
  function startQuiz(initialQuestions?: IdiomRecognitionQuestion[], opts?: { keyReview?: ChineseKeyReviewMeta }) {
  ...
  async function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: idiomQuestionTypeLabel(q.questionType),
      title: q.term,
      correct,
      question: q,
