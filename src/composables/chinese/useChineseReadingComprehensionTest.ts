...
import { isAiChatConfigured, requestReadingComprehensionMcqs, DEEPSEEK_NOT_CONFIGURED_HINT } from '@/services/deepseek'
import {
  appendGeneratedTerms,
  listRecentGeneratedTerms,
  type ChineseGeneratedHistoryKind,
} from '@/utils/chinese/chineseGeneratedHistory'
import { upsertChineseReadingComprehensionWrong } from '@/utils/chinese/chineseReadingComprehensionStorage'
...
import {
  ...
  readingComprehensionHistoryKind,
...
export type ChineseReadingComprehensionResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
...
export function useChineseReadingComprehensionTest(
  selectedMode: Ref<ChineseReadingQuestionType | null>,
) {
  const phase = ref<ChineseReadingComprehensionPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<ReadingComprehensionQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const paperSource = ref<ChinesePaperSource>(null)
  const results = ref<ChineseReadingComprehensionResultRow[]>([])
  ...
  const wrongGate = createChineseWrongBookGate(upsertChineseReadingComprehensionWrong)
  ...
  async function generatePaper() {
    ...
    const historyKind = readingComprehensionHistoryKind(mode) as ChineseGeneratedHistoryKind
    try {
      const generated = await requestReadingComprehensionMcqs({
        count: READING_COMPREHENSION_QUESTION_COUNT,
        mode,
  ...
  function startQuiz(initialQuestions?: ReadingComprehensionQuestion[], opts?: { keyReview?: ChineseKeyReviewMeta }) {
    if (initialQuestions?.length) {
      ...
      if (!selectedMode.value) {
        selectedMode.value = initialQuestions[0]!.questionType
      }
    } else {
  ...
  async function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: readingComprehensionQuestionTypeLabel(q.questionType),
      title: q.term,
      correct,
      question: q,
