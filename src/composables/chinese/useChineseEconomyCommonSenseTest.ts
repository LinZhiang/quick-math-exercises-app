...
import { upsertChineseEconomyCommonSenseWrong } from '@/utils/chinese/chineseEconomyCommonSenseStorage'
...
import {
  ...
  economyCommonSenseQuestionTypeLabel,
...
export type ChineseEconomyCommonSensePhase = 'idle' | 'loading' | 'running' | 'summary'

export type ChineseEconomyCommonSenseResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: EconomyCommonSenseQuestion
...
export function useChineseEconomyCommonSenseTest() {
  const phase = ref<ChineseEconomyCommonSensePhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<EconomyCommonSenseQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const paperSource = ref<ChinesePaperSource>(null)
  const results = ref<ChineseEconomyCommonSenseResultRow[]>([])
  ...
  const wrongGate = createChineseWrongBookGate(upsertChineseEconomyCommonSenseWrong)
  const carelessMarked = ref(false)

  const currentQuestion = computed(() => questions.value[currentIndex.value] ?? null)
  const correctCount = computed(() => results.value.filter((r) => r.correct).length)
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : ECONOMY_COMMON_SENSE_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    try {
      const generated = await requestEconomyCommonSenseMcqs({
        count: ECONOMY_COMMON_SENSE_QUESTION_COUNT,
        avoidTerms: listRecentGeneratedTerms('economy-common-sense'),
        onProgress: (msg) => {
  ...
  function startQuiz(initialQuestions?: EconomyCommonSenseQuestion[], opts?: { keyReview?: ChineseKeyReviewMeta }) {
  ...
  async function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: economyCommonSenseQuestionTypeLabel(q.questionType),
      title: q.term,
      correct,
      question: q,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      incrementPracticeCompletion('chinese-economy-common-sense', {
