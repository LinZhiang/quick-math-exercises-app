...
import { isAiChatConfigured, requestWordMemorizationMcqs, DEEPSEEK_NOT_CONFIGURED_HINT } from '@/services/deepseek'
import {
  appendGeneratedTerms,
  listRecentGeneratedTerms,
} from '@/utils/chinese/chineseGeneratedHistory'
import { upsertChineseWordMemorizationWrong } from '@/utils/chinese/chineseWordMemorizationStorage'
...
export type ChineseWordMemorizationPhase = 'idle' | 'loading' | 'running' | 'summary'

export type ChineseWordMemorizationResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: WordMemorizationQuestion
...
export function useChineseWordMemorizationTest() {
  const phase = ref<ChineseWordMemorizationPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<WordMemorizationQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const paperSource = ref<ChinesePaperSource>(null)
  const results = ref<ChineseWordMemorizationResultRow[]>([])
  ...
  const wrongGate = createChineseWrongBookGate(upsertChineseWordMemorizationWrong)
  const carelessMarked = ref(false)

  const currentQuestion = computed(() => questions.value[currentIndex.value] ?? null)
  const correctCount = computed(() => results.value.filter((r) => r.correct).length)
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : WORD_MEMORIZATION_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    try {
      const generated = await requestWordMemorizationMcqs({
        count: WORD_MEMORIZATION_QUESTION_COUNT,
  ...
  async function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: wordMemorizationQuestionTypeLabel(q.questionType),
      title: q.term,
      correct,
      question: q,
