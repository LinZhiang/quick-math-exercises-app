...
import { isAiChatConfigured, requestCharLiteracyMcqs, DEEPSEEK_NOT_CONFIGURED_HINT } from '@/services/deepseek'
import {
  appendGeneratedTerms,
  listRecentGeneratedTerms,
} from '@/utils/chinese/chineseGeneratedHistory'
import { upsertChineseCharLiteracyWrong } from '@/utils/chinese/chineseCharLiteracyStorage'
...
export function useChineseCharLiteracyTest() {
  ...
  const wrongGate = createChineseWrongBookGate(upsertChineseCharLiteracyWrong)
  ...
  function startQuiz(initialQuestions?: CharLiteracyQuestion[], opts?: { keyReview?: ChineseKeyReviewMeta }) {
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      incrementPracticeCompletion('chinese-char-literacy', {
