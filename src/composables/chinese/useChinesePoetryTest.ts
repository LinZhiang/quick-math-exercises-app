...
import { isAiChatConfigured, requestPoetryRecognitionMcqs, DEEPSEEK_NOT_CONFIGURED_HINT } from '@/services/deepseek'
import {
  appendGeneratedTerms,
  listRecentGeneratedTerms,
} from '@/utils/chinese/chineseGeneratedHistory'
import { upsertChinesePoetryWrong } from '@/utils/chinese/chinesePoetryStorage'
...
export function useChinesePoetryTest() {
  ...
  const wrongGate = createChineseWrongBookGate(upsertChinesePoetryWrong)
  ...
  function startQuiz(initialQuestions?: PoetryRecognitionQuestion[], opts?: { keyReview?: ChineseKeyReviewMeta }) {
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      incrementPracticeCompletion('chinese-poetry', {
