...
import { isAiChatConfigured, requestTheoryPolicyMcqs, DEEPSEEK_NOT_CONFIGURED_HINT } from '@/services/deepseek'
import {
  appendGeneratedTerms,
  listRecentGeneratedTerms,
} from '@/utils/chinese/chineseGeneratedHistory'
import { upsertChineseTheoryPolicyWrong } from '@/utils/chinese/chineseTheoryPolicyStorage'
...
export type ChineseTheoryPolicyPhase = 'idle' | 'loading' | 'running' | 'summary'

export type ChineseTheoryPolicyResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: TheoryPolicyQuestion
...
export function useChineseTheoryPolicyTest() {
  const phase = ref<ChineseTheoryPolicyPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<TheoryPolicyQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const paperSource = ref<ChinesePaperSource>(null)
  const results = ref<ChineseTheoryPolicyResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : THEORY_POLICY_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    try {
      const generated = await requestTheoryPolicyMcqs({
        count: THEORY_POLICY_QUESTION_COUNT,
        avoidTerms: listRecentGeneratedTerms('theory-policy'),
        onProgress: (msg) => {
  ...
  function startQuiz(initialQuestions?: TheoryPolicyQuestion[], opts?: { keyReview?: ChineseKeyReviewMeta }) {
  ...
  async function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: theoryPolicyQuestionTypeLabel(q.questionType),
      title: q.term,
      correct,
      question: q,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      incrementPracticeCompletion('chinese-theory-policy', {
