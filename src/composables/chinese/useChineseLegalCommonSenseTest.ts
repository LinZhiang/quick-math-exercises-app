...
import { isAiChatConfigured, requestLegalCommonSenseMcqs, DEEPSEEK_NOT_CONFIGURED_HINT } from '@/services/deepseek'
import {
  appendGeneratedTerms,
  listRecentGeneratedTerms,
} from '@/utils/chinese/chineseGeneratedHistory'
import { upsertChineseLegalCommonSenseWrong } from '@/utils/chinese/chineseLegalCommonSenseStorage'
...
export function useChineseLegalCommonSenseTest() {
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      incrementPracticeCompletion('chinese-legal-common-sense', {
