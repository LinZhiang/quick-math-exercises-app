...
import {
  ...
  isAiChatConfigured,
  requestDataAnalysisGrowthAvgAnnualMcqs,
} from '@/services/deepseek'
import {
  appendGeneratedTerms,
  listRecentGeneratedTerms,
} from '@/utils/chinese/chineseGeneratedHistory'
...
import { playMentalMathStartSound } from '@/utils/math/mentalMathSounds'
import {
...
import { formatDataAnalysisMathPlain } from '@/utils/data-analysis/dataAnalysisMathDisplay'
...
export type GrowthAvgAnnualResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: GrowthAvgAnnualQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  const chartLine = q.chart?.title ? `【图】${q.chart.title}` : ''
  const tableLine = q.table?.title ? `【表】${q.table.title}` : ''
  const expression = [q.passage, chartLine, tableLine, q.stem].filter(Boolean).join('\n\n')
  const explainParts = [q.method ? `做法：${q.method}` : '', q.explanation]
    .map((s) => s.trim())
    .filter(Boolean)
  upsertMentalMathWrong({
    modeId: `data-analysis-growth-avg-annual-${q.difficulty}`,
    expression,
    correctAnswer: formatDataAnalysisMathPlain(q.options[q.correctIndex] ?? ''),
    chosenAnswer: formatDataAnalysisMathPlain(q.options[pending.chosenIndex] ?? ''),
    options: q.options.map((o) => formatDataAnalysisMathPlain(o)),
    explanation: formatDataAnalysisMathPlain(explainParts.join('\n')) || undefined,
  })
}

export function useDataAnalysisGrowthAvgAnnualTest(
  difficulty: Ref<GrowthAvgAnnualDifficulty | null>,
) {
  const phase = ref<GrowthAvgAnnualPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<GrowthAvgAnnualQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<GrowthAvgAnnualResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : GROWTH_AVG_ANNUAL_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    loadingMessage.value =
      diff === 'hard' ? '豆包生成年均增长复杂题（图/表）…' : '豆包生成年均增长题目…'
    try {
      const generated = await requestDataAnalysisGrowthAvgAnnualMcqs({
        count: GROWTH_AVG_ANNUAL_QUESTION_COUNT,
        difficulty: diff,
        avoidTerms: listRecentGeneratedTerms('data-analysis-growth-avg-annual'),
        onProgress: (msg) => {
      ...
      phase.value = 'idle'
      ElMessage.success(`已生成 ${questions.value.length} 道题（豆包）`)
    } catch (e) {
      phase.value = 'idle'
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${growthAvgAnnualTopicLabel()} · ${growthAvgAnnualDifficultyLabel(q.difficulty)}`,
