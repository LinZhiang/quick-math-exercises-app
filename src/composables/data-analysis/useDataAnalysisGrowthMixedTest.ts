...
import {
  ...
  isAiChatConfigured,
  requestDataAnalysisGrowthMixedMcqs,
...
import {
  ...
  growthMixedDifficultyLabel,
  growthMixedTopicLabel,
...
export type GrowthMixedPhase = 'idle' | 'loading' | 'running' | 'summary'

export type GrowthMixedResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: GrowthMixedQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `data-analysis-growth-mixed-${q.difficulty}`,
    expression,
...
export function useDataAnalysisGrowthMixedTest(
  difficulty: Ref<GrowthMixedDifficulty | null>,
) {
  const phase = ref<GrowthMixedPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<GrowthMixedQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<GrowthMixedResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : GROWTH_MIXED_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    loadingMessage.value =
      diff === 'hard'
        ? '豆包整包生成混合增长复杂题（失败即用本地题库，不会长时间卡住）…'
        : '豆包生成混合增长题目…'
    try {
      const generated = await requestDataAnalysisGrowthMixedMcqs({
        count: GROWTH_MIXED_QUESTION_COUNT,
        difficulty: diff,
        avoidTerms: listRecentGeneratedTerms('data-analysis-growth-mixed'),
        onProgress: (msg) => {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${growthMixedTopicLabel()} · ${growthMixedDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`data-analysis-growth-mixed-${difficulty.value}`, {
