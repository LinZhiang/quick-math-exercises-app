...
import {
  ...
  isAiChatConfigured,
  requestDataAnalysisProportionBaseMcqs,
...
export type ProportionBaseResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: ProportionBaseQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `data-analysis-proportion-base-${q.difficulty}`,
    expression,
...
export function useDataAnalysisProportionBaseTest(
  difficulty: Ref<ProportionBaseDifficulty | null>,
) {
  const phase = ref<ProportionBasePhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<ProportionBaseQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<ProportionBaseResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : PROPORTION_BASE_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    loadingMessage.value =
      diff === 'hard'
        ? '豆包整包生成基期比重复杂题（失败即用本地题库，不会长时间卡住）…'
        : '豆包生成基期比重题目…'
    try {
      const generated = await requestDataAnalysisProportionBaseMcqs({
        count: PROPORTION_BASE_QUESTION_COUNT,
        difficulty: diff,
        avoidTerms: listRecentGeneratedTerms('data-analysis-proportion-base'),
        onProgress: (msg) => {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${proportionBaseTopicLabel()} · ${proportionBaseDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`data-analysis-proportion-base-${difficulty.value}`, {
