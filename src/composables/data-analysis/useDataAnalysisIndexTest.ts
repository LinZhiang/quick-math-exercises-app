...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `data-analysis-index-${q.difficulty}`,
    expression,
...
export function useDataAnalysisIndexTest(
  difficulty: Ref<IndexDifficulty | null>,
) {
  const phase = ref<IndexPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<IndexQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<IndexResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : INDEX_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    loadingMessage.value =
      diff === 'hard'
        ? '豆包整包生成指数复杂题（失败即用本地题库，不会长时间卡住）…'
        : '豆包生成指数题目…'
    try {
      const generated = await requestDataAnalysisIndexMcqs({
        count: INDEX_QUESTION_COUNT,
        difficulty: diff,
        avoidTerms: listRecentGeneratedTerms('data-analysis-index'),
        onProgress: (msg) => {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${indexTopicLabel()} · ${indexDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`data-analysis-index-${difficulty.value}`, {
