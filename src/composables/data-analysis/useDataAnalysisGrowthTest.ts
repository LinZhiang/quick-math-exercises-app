...
import {
  ...
  isAiChatConfigured,
  requestDataAnalysisGrowthGeneralMcqs,
...
export type GrowthGeneralResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: GrowthGeneralQuestion
...
function saveGrowthWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `data-analysis-growth-${q.difficulty}`,
    expression,
...
export function useDataAnalysisGrowthTest(difficulty: Ref<GrowthGeneralDifficulty | null>) {
  const phase = ref<GrowthGeneralPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<GrowthGeneralQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<GrowthGeneralResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : GROWTH_GENERAL_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    loadingMessage.value =
      diff === 'hard' ? '豆包生成复杂题（含统计图）…' : '豆包生成一般增长题目…'
    try {
      const generated = await requestDataAnalysisGrowthGeneralMcqs({
        count: GROWTH_GENERAL_QUESTION_COUNT,
        difficulty: diff,
        avoidTerms: listRecentGeneratedTerms('data-analysis-growth'),
        onProgress: (msg) => {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${growthGeneralTopicLabel()} · ${growthGeneralDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`data-analysis-growth-${difficulty.value}`, {
