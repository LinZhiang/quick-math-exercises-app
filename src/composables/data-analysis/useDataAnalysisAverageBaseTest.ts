...
import {
  ...
  isAiChatConfigured,
  requestDataAnalysisAverageBaseMcqs,
...
import {
  ...
  averageBaseDifficultyLabel,
  averageBaseTopicLabel,
...
export type AverageBasePhase = 'idle' | 'loading' | 'running' | 'summary'

export type AverageBaseResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: AverageBaseQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `data-analysis-average-base-${q.difficulty}`,
    expression,
...
export function useDataAnalysisAverageBaseTest(
  difficulty: Ref<AverageBaseDifficulty | null>,
) {
  const phase = ref<AverageBasePhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<AverageBaseQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<AverageBaseResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : AVERAGE_BASE_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    loadingMessage.value =
      diff === 'hard'
        ? '豆包整包生成基期平均数复杂题（失败即用本地题库，不会长时间卡住）…'
        : '豆包生成基期平均数题目…'
    try {
      const generated = await requestDataAnalysisAverageBaseMcqs({
        count: AVERAGE_BASE_QUESTION_COUNT,
        difficulty: diff,
        avoidTerms: listRecentGeneratedTerms('data-analysis-average-base'),
        onProgress: (msg) => {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${averageBaseTopicLabel()} · ${averageBaseDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`data-analysis-average-base-${difficulty.value}`, {
