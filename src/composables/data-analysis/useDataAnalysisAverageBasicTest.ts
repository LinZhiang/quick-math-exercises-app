...
import {
  ...
  isAiChatConfigured,
  requestDataAnalysisAverageBasicMcqs,
...
import {
  ...
  averageBasicDifficultyLabel,
  averageBasicTopicLabel,
...
export type AverageBasicPhase = 'idle' | 'loading' | 'running' | 'summary'

export type AverageBasicResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: AverageBasicQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `data-analysis-average-basic-${q.difficulty}`,
    expression,
...
export function useDataAnalysisAverageBasicTest(
  difficulty: Ref<AverageBasicDifficulty | null>,
) {
  const phase = ref<AverageBasicPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<AverageBasicQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<AverageBasicResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : AVERAGE_BASIC_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    loadingMessage.value =
      diff === 'hard'
        ? '豆包整包生成平均数基本公式复杂题（失败即用本地题库，不会长时间卡住）…'
        : '豆包生成平均数基本公式题目…'
    try {
      const generated = await requestDataAnalysisAverageBasicMcqs({
        count: AVERAGE_BASIC_QUESTION_COUNT,
        difficulty: diff,
        avoidTerms: listRecentGeneratedTerms('data-analysis-average-basic'),
        onProgress: (msg) => {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${averageBasicTopicLabel()} · ${averageBasicDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`data-analysis-average-basic-${difficulty.value}`, {
