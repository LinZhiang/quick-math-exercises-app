...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `data-analysis-pull-${q.difficulty}`,
    expression,
...
export function useDataAnalysisPullTest(
  difficulty: Ref<PullDifficulty | null>,
) {
  const phase = ref<PullPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<PullQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<PullResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : PULL_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    loadingMessage.value =
      diff === 'hard'
        ? '豆包整包生成拉动增长和比例复杂题（失败即用本地题库，不会长时间卡住）…'
        : '豆包生成拉动增长和比例题目…'
    try {
      const generated = await requestDataAnalysisPullMcqs({
        count: PULL_QUESTION_COUNT,
        difficulty: diff,
        avoidTerms: listRecentGeneratedTerms('data-analysis-pull'),
        onProgress: (msg) => {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${pullTopicLabel()} · ${pullDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`data-analysis-pull-${difficulty.value}`, {
