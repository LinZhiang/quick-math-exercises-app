...
import {
  ...
  isAiChatConfigured,
  requestDataAnalysisProportionBasicMcqs,
...
import {
  ...
  proportionBasicDifficultyLabel,
  proportionBasicTopicLabel,
...
export type ProportionBasicPhase = 'idle' | 'loading' | 'running' | 'summary'

export type ProportionBasicResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: ProportionBasicQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `data-analysis-proportion-basic-${q.difficulty}`,
    expression,
...
export function useDataAnalysisProportionBasicTest(
  difficulty: Ref<ProportionBasicDifficulty | null>,
) {
  const phase = ref<ProportionBasicPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<ProportionBasicQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<ProportionBasicResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : PROPORTION_BASIC_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    loadingMessage.value =
      diff === 'hard'
        ? '豆包整包生成比重基本公式复杂题（失败即用本地题库，不会长时间卡住）…'
        : '豆包生成比重基本公式题目…'
    try {
      const generated = await requestDataAnalysisProportionBasicMcqs({
        count: PROPORTION_BASIC_QUESTION_COUNT,
        difficulty: diff,
        avoidTerms: listRecentGeneratedTerms('data-analysis-proportion-basic'),
        onProgress: (msg) => {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${proportionBasicTopicLabel()} · ${proportionBasicDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`data-analysis-proportion-basic-${difficulty.value}`, {
