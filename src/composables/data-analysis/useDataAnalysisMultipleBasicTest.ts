...
import {
  ...
  isAiChatConfigured,
  requestDataAnalysisMultipleBasicMcqs,
...
export type MultipleBasicPhase = 'idle' | 'loading' | 'running' | 'summary'

export type MultipleBasicResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: MultipleBasicQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `data-analysis-multiple-basic-${q.difficulty}`,
    expression,
...
export function useDataAnalysisMultipleBasicTest(
  difficulty: Ref<MultipleBasicDifficulty | null>,
) {
  const phase = ref<MultipleBasicPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<MultipleBasicQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<MultipleBasicResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : MULTIPLE_BASIC_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    loadingMessage.value =
      diff === 'hard'
        ? '豆包整包生成倍数基本公式复杂题（失败即用本地题库，不会长时间卡住）…'
        : '豆包生成倍数基本公式题目…'
    try {
      const generated = await requestDataAnalysisMultipleBasicMcqs({
        count: MULTIPLE_BASIC_QUESTION_COUNT,
        difficulty: diff,
        avoidTerms: listRecentGeneratedTerms('data-analysis-multiple-basic'),
        onProgress: (msg) => {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${multipleBasicTopicLabel()} · ${multipleBasicDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`data-analysis-multiple-basic-${difficulty.value}`, {
