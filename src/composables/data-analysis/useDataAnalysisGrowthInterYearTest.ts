...
import {
  ...
  isAiChatConfigured,
  requestDataAnalysisGrowthInterYearMcqs,
...
export type GrowthInterYearResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: GrowthInterYearQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  const expression = [q.passage, chartLine, q.stem].filter(Boolean).join('\n\n')
  const explainParts = [q.method ? `做法：${q.method}` : '', q.explanation]
    .map((s) => s.trim())
    .filter(Boolean)
  upsertMentalMathWrong({
    modeId: `data-analysis-growth-inter-year-${q.difficulty}`,
    expression,
...
export function useDataAnalysisGrowthInterYearTest(
  difficulty: Ref<GrowthInterYearDifficulty | null>,
) {
  const phase = ref<GrowthInterYearPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<GrowthInterYearQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<GrowthInterYearResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : GROWTH_INTER_YEAR_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    loadingMessage.value =
      diff === 'hard' ? '豆包生成隔年增长复杂题（双折线图）…' : '豆包生成隔年增长题目…'
    try {
      const generated = await requestDataAnalysisGrowthInterYearMcqs({
        count: GROWTH_INTER_YEAR_QUESTION_COUNT,
        difficulty: diff,
        avoidTerms: listRecentGeneratedTerms('data-analysis-growth-inter-year'),
        onProgress: (msg) => {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${growthInterYearTopicLabel()} · ${growthInterYearDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`data-analysis-growth-inter-year-${difficulty.value}`, {
