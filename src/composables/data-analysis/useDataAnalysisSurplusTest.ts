...
import {
  ...
  isAiChatConfigured,
  requestDataAnalysisSurplusMcqs,
...
export type SurplusResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: SurplusQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `data-analysis-surplus-${q.difficulty}`,
    expression,
...
export function useDataAnalysisSurplusTest(
  difficulty: Ref<SurplusDifficulty | null>,
) {
  const phase = ref<SurplusPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<SurplusQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<SurplusResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : SURPLUS_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    loadingMessage.value =
      diff === 'hard'
        ? '豆包整包生成顺差与逆差复杂题（失败即用本地题库，不会长时间卡住）…'
        : '豆包生成顺差与逆差题目…'
    try {
      const generated = await requestDataAnalysisSurplusMcqs({
        count: SURPLUS_QUESTION_COUNT,
        difficulty: diff,
        avoidTerms: listRecentGeneratedTerms('data-analysis-surplus'),
        onProgress: (msg) => {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${surplusTopicLabel()} · ${surplusDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`data-analysis-surplus-${difficulty.value}`, {
