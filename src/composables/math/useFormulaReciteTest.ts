...
import {
  ...
  generateFormulaRecitePaper,
  formulaReciteDifficultyLabel,
  formulaReciteModuleLabel,
  type FormulaReciteModuleId,
  type FormulaReciteQuestion,
} from '@/utils/math/formulaRecitePractice'
import { incrementPracticeCompletion } from '@/utils/app/practiceCompletionStats'

export type FormulaRecitePhase = 'idle' | 'loading' | 'running' | 'summary'

export type FormulaReciteResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: FormulaReciteQuestion
...
function modeIdFor(moduleId: FormulaReciteModuleId): string {
  return `formula-recite-${moduleId}`
}
...
export function useFormulaReciteTest(moduleId: Ref<FormulaReciteModuleId | null>) {
  const phase = ref<FormulaRecitePhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<FormulaReciteQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<FormulaReciteResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : FORMULA_RECITE_QUESTION_COUNT,
  )
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${formulaReciteModuleLabel(q.moduleId)} · ${formulaReciteDifficultyLabel()}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (moduleId.value) {
        incrementPracticeCompletion(modeIdFor(moduleId.value), {
