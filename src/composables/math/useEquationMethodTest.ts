...
export type EqMethodResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: EqMethodQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-skill-eq-method-${q.difficulty}`,
    expression,
...
export function useEquationMethodTest(difficulty: Ref<EqMethodDifficulty | null>) {
  const phase = ref<EqMethodPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<EqMethodQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<EqMethodResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : EQ_METHOD_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateEqMethodPaper(diff)
      if (generated.length < EQ_METHOD_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${eqMethodTopicLabel()} · ${eqMethodDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`op-skill-eq-method-${difficulty.value}`, {
