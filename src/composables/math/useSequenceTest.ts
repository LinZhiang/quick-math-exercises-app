...
import {
  SEQUENCE_QUESTION_COUNT,
  generateSequencePaper,
...
export type SequencePhase = 'idle' | 'loading' | 'running' | 'summary'

export type SequenceResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-other-sequence-${q.difficulty}`,
    expression,
...
export function useSequenceTest(difficulty: Ref<SequenceDifficulty | null>) {
  const phase = ref<SequencePhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<SequenceQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<SequenceResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : SEQUENCE_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateSequencePaper(diff)
      if (generated.length < SEQUENCE_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${sequenceTopicLabel()} · ${sequenceDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`op-other-sequence-${difficulty.value}`, {
