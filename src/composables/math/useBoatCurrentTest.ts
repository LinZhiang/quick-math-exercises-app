...
import {
  BOAT_CURRENT_QUESTION_COUNT,
  boatCurrentDifficultyLabel,
  boatCurrentTopicLabel,
  generateBoatCurrentPaper,
...
export type BoatCurrentPhase = 'idle' | 'loading' | 'running' | 'summary'

export type BoatCurrentResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: BoatCurrentQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-highfreq-boat-current-${q.difficulty}`,
    expression,
...
export function useBoatCurrentTest(difficulty: Ref<BoatCurrentDifficulty | null>) {
  const phase = ref<BoatCurrentPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<BoatCurrentQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<BoatCurrentResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : BOAT_CURRENT_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateBoatCurrentPaper(diff)
      if (generated.length < BOAT_CURRENT_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${boatCurrentTopicLabel()} · ${boatCurrentDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`op-highfreq-boat-current-${difficulty.value}`, {
