...
import {
  ORDINARY_TRAVEL_QUESTION_COUNT,
  ordinaryTravelDifficultyLabel,
  ordinaryTravelTopicLabel,
  generateOrdinaryTravelPaper,
...
export type OrdinaryTravelPhase = 'idle' | 'loading' | 'running' | 'summary'

export type OrdinaryTravelResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: OrdinaryTravelQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-highfreq-ordinary-travel-${q.difficulty}`,
    expression,
...
export function useOrdinaryTravelTest(difficulty: Ref<OrdinaryTravelDifficulty | null>) {
  const phase = ref<OrdinaryTravelPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<OrdinaryTravelQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<OrdinaryTravelResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : ORDINARY_TRAVEL_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateOrdinaryTravelPaper(diff)
      if (generated.length < ORDINARY_TRAVEL_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${ordinaryTravelTopicLabel()} · ${ordinaryTravelDifficultyLabel(q.difficulty)}`,
