...
import {
  PROFIT_CALC_QUESTION_COUNT,
  profitCalcDifficultyLabel,
  profitCalcTopicLabel,
  generateProfitCalcPaper,
...
export type ProfitCalcPhase = 'idle' | 'loading' | 'running' | 'summary'

export type ProfitCalcResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: ProfitCalcQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-highfreq-profit-calc-${q.difficulty}`,
    expression,
...
export function useProfitCalcTest(difficulty: Ref<ProfitCalcDifficulty | null>) {
  const phase = ref<ProfitCalcPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<ProfitCalcQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<ProfitCalcResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : PROFIT_CALC_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateProfitCalcPaper(diff)
      if (generated.length < PROFIT_CALC_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${profitCalcTopicLabel()} · ${profitCalcDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`op-highfreq-profit-calc-${difficulty.value}`, {
