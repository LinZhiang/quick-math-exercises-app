...
import {
  PERM_COMB_CONSTRAINT_QUESTION_COUNT,
  permCombConstraintDifficultyLabel,
  permCombConstraintTopicLabel,
  generatePermCombConstraintPaper,
...
export type PermCombConstraintPhase = 'idle' | 'loading' | 'running' | 'summary'

export type PermCombConstraintResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: PermCombConstraintQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-highfreq-perm-comb-constraint-${q.difficulty}`,
    expression,
...
export function usePermCombConstraintTest(difficulty: Ref<PermCombConstraintDifficulty | null>) {
  const phase = ref<PermCombConstraintPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<PermCombConstraintQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<PermCombConstraintResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : PERM_COMB_CONSTRAINT_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generatePermCombConstraintPaper(diff)
      if (generated.length < PERM_COMB_CONSTRAINT_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${permCombConstraintTopicLabel()} · ${permCombConstraintDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`op-highfreq-perm-comb-constraint-${difficulty.value}`, {
