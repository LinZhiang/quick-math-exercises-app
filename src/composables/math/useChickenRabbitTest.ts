...
import {
  CHICKEN_RABBIT_QUESTION_COUNT,
  generateChickenRabbitPaper,
  chickenRabbitDifficultyLabel,
  chickenRabbitTopicLabel,
  type ChickenRabbitDifficulty,
  type ChickenRabbitQuestion,
} from '@/utils/math/chickenRabbitPractice'
import { incrementPracticeCompletion } from '@/utils/app/practiceCompletionStats'

export type ChickenRabbitPhase = 'idle' | 'loading' | 'running' | 'summary'

export type ChickenRabbitResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: ChickenRabbitQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-other-chicken-rabbit-${q.difficulty}`,
    expression,
...
export function useChickenRabbitTest(difficulty: Ref<ChickenRabbitDifficulty | null>) {
  const phase = ref<ChickenRabbitPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<ChickenRabbitQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<ChickenRabbitResultRow[]>([])
  ...
  let quizWallStartMs: number | null = null
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : CHICKEN_RABBIT_QUESTION_COUNT,
  )
  ...
  function getRunningElapsedMs(): number {
    if (quizWallStartMs == null) return quizRunningDisplayMs.value
    let paused = totalPausedMs
    if (pauseStartMs != null) paused += Math.round(performance.now() - pauseStartMs)
    return Math.max(0, Math.round(performance.now() - quizWallStartMs - paused))
  }
  ...
  function startQuizTimer() {
    ...
    quizWallStartMs = performance.now()
    totalPausedMs = 0
    pauseStartMs = null
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateChickenRabbitPaper(diff)
      if (generated.length < CHICKEN_RABBIT_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${chickenRabbitTopicLabel()} · ${chickenRabbitDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`op-other-chicken-rabbit-${difficulty.value}`, {
