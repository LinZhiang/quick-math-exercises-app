...
export type GcdLcmResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: GcdLcmQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-skill-gcd-lcm-${q.difficulty}`,
    expression,
...
export function useGcdLcmTest(difficulty: Ref<GcdLcmDifficulty | null>) {
  const phase = ref<GcdLcmPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<GcdLcmQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<GcdLcmResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : GCD_LCM_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateGcdLcmPaper(diff)
      if (generated.length < GCD_LCM_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${gcdLcmTopicLabel()} · ${gcdLcmDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`op-skill-gcd-lcm-${difficulty.value}`, {
