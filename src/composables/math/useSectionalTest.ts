...
import {
  SECTIONAL_QUESTION_COUNT,
  generateSectionalPaper,
  sectionalDifficultyLabel,
  sectionalTopicLabel,
  type SectionalDifficulty,
  type SectionalQuestion,
} from '@/utils/math/sectionalPractice'
import { incrementPracticeCompletion } from '@/utils/app/practiceCompletionStats'

export type SectionalPhase = 'idle' | 'loading' | 'running' | 'summary'

export type SectionalResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-other-sectional-${q.difficulty}`,
    expression,
...
export function useSectionalTest(difficulty: Ref<SectionalDifficulty | null>) {
  const phase = ref<SectionalPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<SectionalQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<SectionalResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : SECTIONAL_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateSectionalPaper(diff)
      if (generated.length < SECTIONAL_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${sectionalTopicLabel()} · ${sectionalDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`op-other-sectional-${difficulty.value}`, {
