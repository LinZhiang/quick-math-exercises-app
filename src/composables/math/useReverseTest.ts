...
import {
  REVERSE_QUESTION_COUNT,
  generateReversePaper,
...
export type ReversePhase = 'idle' | 'loading' | 'running' | 'summary'

export type ReverseResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-other-reverse-${q.difficulty}`,
    expression,
...
export function useReverseTest(difficulty: Ref<ReverseDifficulty | null>) {
  const phase = ref<ReversePhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<ReverseQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<ReverseResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : REVERSE_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateReversePaper(diff)
      if (generated.length < REVERSE_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${reverseTopicLabel()} · ${reverseDifficultyLabel(q.difficulty)}`,
