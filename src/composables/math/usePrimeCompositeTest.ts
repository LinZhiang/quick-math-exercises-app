...
import {
  ...
  primeCompDifficultyLabel,
  primeCompTopicLabel,
  generatePrimeCompositePaper,
  type PrimeCompDifficulty,
  type PrimeCompQuestion,
} from '@/utils/math/primeCompositePractice'
...
export type PrimeCompResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: PrimeCompQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-skill-prime-comp-${q.difficulty}`,
    expression,
...
export function usePrimeCompositeTest(difficulty: Ref<PrimeCompDifficulty | null>) {
  const phase = ref<PrimeCompPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<PrimeCompQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<PrimeCompResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : PRIME_COMP_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generatePrimeCompositePaper(diff)
      if (generated.length < PRIME_COMP_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${primeCompTopicLabel()} · ${primeCompDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`op-skill-prime-comp-${difficulty.value}`, {
