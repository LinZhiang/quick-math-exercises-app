...
import {
  RIGHT_TRIANGLE_QUESTION_COUNT,
  rightTriangleDifficultyLabel,
  rightTriangleTopicLabel,
  generateRightTrianglePaper,
...
export type RightTrianglePhase = 'idle' | 'loading' | 'running' | 'summary'

export type RightTriangleResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: RightTriangleQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-highfreq-right-triangle-${q.difficulty}`,
    expression,
...
export function useRightTriangleTest(difficulty: Ref<RightTriangleDifficulty | null>) {
  const phase = ref<RightTrianglePhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<RightTriangleQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<RightTriangleResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : RIGHT_TRIANGLE_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateRightTrianglePaper(diff)
      if (generated.length < RIGHT_TRIANGLE_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${rightTriangleTopicLabel()} · ${rightTriangleDifficultyLabel(q.difficulty)}`,
