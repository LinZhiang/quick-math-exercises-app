...
import {
  ORDINARY_WORK_QUESTION_COUNT,
  ordinaryWorkDifficultyLabel,
  ordinaryWorkTopicLabel,
  generateOrdinaryWorkPaper,
...
export type OrdinaryWorkPhase = 'idle' | 'loading' | 'running' | 'summary'

export type OrdinaryWorkResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: OrdinaryWorkQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-highfreq-ordinary-work-${q.difficulty}`,
    expression,
...
export function useOrdinaryWorkTest(difficulty: Ref<OrdinaryWorkDifficulty | null>) {
  const phase = ref<OrdinaryWorkPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<OrdinaryWorkQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<OrdinaryWorkResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : ORDINARY_WORK_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateOrdinaryWorkPaper(diff)
      if (generated.length < ORDINARY_WORK_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${ordinaryWorkTopicLabel()} · ${ordinaryWorkDifficultyLabel(q.difficulty)}`,
