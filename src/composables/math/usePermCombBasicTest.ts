...
import {
  PERM_COMB_BASIC_QUESTION_COUNT,
  permCombBasicDifficultyLabel,
  permCombBasicTopicLabel,
  generatePermCombBasicPaper,
  type PermCombBasicDifficulty,
  type PermCombBasicQuestion,
} from '@/utils/math/permCombBasicPractice'
import { incrementPracticeCompletion } from '@/utils/app/practiceCompletionStats'

export type PermCombBasicPhase = 'idle' | 'loading' | 'running' | 'summary'

export type PermCombBasicResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: PermCombBasicQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-highfreq-perm-comb-basic-${q.difficulty}`,
    expression,
...
export function usePermCombBasicTest(difficulty: Ref<PermCombBasicDifficulty | null>) {
  const phase = ref<PermCombBasicPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<PermCombBasicQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<PermCombBasicResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : PERM_COMB_BASIC_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generatePermCombBasicPaper(diff)
      if (generated.length < PERM_COMB_BASIC_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${permCombBasicTopicLabel()} · ${permCombBasicDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`op-highfreq-perm-comb-basic-${difficulty.value}`, {
