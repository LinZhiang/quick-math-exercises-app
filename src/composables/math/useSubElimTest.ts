...
export type SubElimResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: SubElimQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-skill-sub-elim-${q.difficulty}`,
    expression,
...
export function useSubElimTest(difficulty: Ref<SubElimDifficulty | null>) {
  const phase = ref<SubElimPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<SubElimQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<SubElimResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : SUB_ELIM_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateSubElimPaper(diff)
      if (generated.length < SUB_ELIM_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${subElimTopicLabel()} · ${subElimDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`op-skill-sub-elim-${difficulty.value}`, {
