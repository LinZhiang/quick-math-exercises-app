...
export type GeometryPhase = 'idle' | 'loading' | 'running' | 'summary'

export type GeometryResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-highfreq-geometry-${q.difficulty}`,
    expression,
...
export function useGeometryTest(difficulty: Ref<GeometryDifficulty | null>) {
  const phase = ref<GeometryPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<GeometryQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<GeometryResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : GEOMETRY_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    phase.value = 'loading'
    loadingMessage.value = '豆包按几何图库出题（失败即用本地同图题）…'
    try {
      const generated = await requestGeometryMcqs({
        count: GEOMETRY_QUESTION_COUNT,
        difficulty: diff,
        avoidTerms: listRecentGeneratedTerms('op-highfreq-geometry'),
        onProgress: (msg) => {
          loadingMessage.value = msg
        },
      })
      if (generated.length < GEOMETRY_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${geometryTopicLabel()} · ${geometryDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`op-highfreq-geometry-${difficulty.value}`, {
