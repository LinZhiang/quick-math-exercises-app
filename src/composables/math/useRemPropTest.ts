...
export type RemPropResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: RemPropQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-skill-rem-prop-${q.difficulty}`,
    expression,
...
export function useRemPropTest(difficulty: Ref<RemPropDifficulty | null>) {
  const phase = ref<RemPropPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<RemPropQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<RemPropResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : REM_PROP_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateRemPropPaper(diff)
      if (generated.length < REM_PROP_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${remPropTopicLabel()} · ${remPropDifficultyLabel(q.difficulty)}`,
