...
import {
  COOPERATIVE_WORK_QUESTION_COUNT,
  cooperativeWorkDifficultyLabel,
  cooperativeWorkTopicLabel,
  generateCooperativeWorkPaper,
...
export type CooperativeWorkPhase = 'idle' | 'loading' | 'running' | 'summary'

export type CooperativeWorkResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: CooperativeWorkQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-highfreq-cooperative-work-${q.difficulty}`,
    expression,
...
export function useCooperativeWorkTest(difficulty: Ref<CooperativeWorkDifficulty | null>) {
  const phase = ref<CooperativeWorkPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<CooperativeWorkQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<CooperativeWorkResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : COOPERATIVE_WORK_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateCooperativeWorkPaper(diff)
      if (generated.length < COOPERATIVE_WORK_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${cooperativeWorkTopicLabel()} · ${cooperativeWorkDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`op-highfreq-cooperative-work-${difficulty.value}`, {
