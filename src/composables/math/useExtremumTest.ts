...
import {
  EXTREMUM_QUESTION_COUNT,
  generateExtremumPaper,
...
export type ExtremumPhase = 'idle' | 'loading' | 'running' | 'summary'

export type ExtremumResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: ExtremumQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-other-extremum-${q.difficulty}`,
    expression,
...
export function useExtremumTest(difficulty: Ref<ExtremumDifficulty | null>) {
  const phase = ref<ExtremumPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<ExtremumQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<ExtremumResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : EXTREMUM_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateExtremumPaper(diff)
      if (generated.length < EXTREMUM_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${extremumTopicLabel()} · ${extremumDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`op-other-extremum-${difficulty.value}`, {
