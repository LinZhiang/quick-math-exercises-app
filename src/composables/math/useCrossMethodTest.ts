...
import {
  CROSS_METHOD_QUESTION_COUNT,
  crossMethodDifficultyLabel,
  crossMethodTopicLabel,
  generateCrossMethodPaper,
...
export type CrossMethodResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: CrossMethodQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-skill-cross-method-${q.difficulty}`,
    expression,
...
export function useCrossMethodTest(difficulty: Ref<CrossMethodDifficulty | null>) {
  const phase = ref<CrossMethodPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<CrossMethodQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<CrossMethodResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : CROSS_METHOD_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateCrossMethodPaper(diff)
      if (generated.length < CROSS_METHOD_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${crossMethodTopicLabel()} · ${crossMethodDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`op-skill-cross-method-${difficulty.value}`, {
