...
import {
  SPEC_VAL_QUESTION_COUNT,
  specValDifficultyLabel,
  specValTopicLabel,
  generateSpecValPaper,
...
export type SpecValResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: SpecValQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-skill-spec-val-${q.difficulty}`,
    expression,
...
export function useSpecialValueTest(difficulty: Ref<SpecValDifficulty | null>) {
  const phase = ref<SpecValPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<SpecValQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<SpecValResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : SPEC_VAL_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateSpecValPaper(diff)
      if (generated.length < SPEC_VAL_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${specValTopicLabel()} · ${specValDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`op-skill-spec-val-${difficulty.value}`, {
