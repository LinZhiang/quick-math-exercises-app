...
import {
  SUM_DIFF_RATIO_QUESTION_COUNT,
  sumDiffRatioDifficultyLabel,
  sumDiffRatioTopicLabel,
  generateSumDiffRatioPaper,
...
export type SumDiffRatioPhase = 'idle' | 'loading' | 'running' | 'summary'

export type SumDiffRatioResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: SumDiffRatioQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-highfreq-sum-diff-ratio-${q.difficulty}`,
    expression,
...
export function useSumDiffRatioTest(difficulty: Ref<SumDiffRatioDifficulty | null>) {
  const phase = ref<SumDiffRatioPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<SumDiffRatioQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<SumDiffRatioResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : SUM_DIFF_RATIO_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateSumDiffRatioPaper(diff)
      if (generated.length < SUM_DIFF_RATIO_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${sumDiffRatioTopicLabel()} · ${sumDiffRatioDifficultyLabel(q.difficulty)}`,
