...
export type ClockPhase = 'idle' | 'loading' | 'running' | 'summary'

export type ClockResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-other-clock-${q.difficulty}`,
    expression,
...
export function useClockTest(difficulty: Ref<ClockDifficulty | null>) {
  const phase = ref<ClockPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<ClockQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<ClockResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : CLOCK_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateClockPaper(diff)
      if (generated.length < CLOCK_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${clockTopicLabel()} · ${clockDifficultyLabel(q.difficulty)}`,
