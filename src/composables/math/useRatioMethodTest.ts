...
import {
  RATIO_METHOD_QUESTION_COUNT,
  ratioMethodDifficultyLabel,
  ratioMethodTopicLabel,
  generateRatioMethodPaper,
...
export type RatioMethodResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: RatioMethodQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-skill-ratio-method-${q.difficulty}`,
    expression,
...
export function useRatioMethodTest(difficulty: Ref<RatioMethodDifficulty | null>) {
  const phase = ref<RatioMethodPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<RatioMethodQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<RatioMethodResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : RATIO_METHOD_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateRatioMethodPaper(diff)
      if (generated.length < RATIO_METHOD_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${ratioMethodTopicLabel()} · ${ratioMethodDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`op-skill-ratio-method-${difficulty.value}`, {
