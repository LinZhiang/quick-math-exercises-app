...
export type CompetitionPhase = 'idle' | 'loading' | 'running' | 'summary'
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-other-competition-${q.difficulty}`,
    expression,
...
export function useCompetitionTest(difficulty: Ref<CompetitionDifficulty | null>) {
  const phase = ref<CompetitionPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<CompetitionQuestion[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : COMPETITION_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateCompetitionPaper(diff)
      if (generated.length < COMPETITION_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${competitionTopicLabel()} · ${competitionDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`op-other-competition-${difficulty.value}`, {
