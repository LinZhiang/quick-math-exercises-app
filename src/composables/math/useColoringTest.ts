...
import {
  COLORING_QUESTION_COUNT,
  coloringDifficultyLabel,
  coloringTopicLabel,
  generateColoringPaper,
...
export type ColoringPhase = 'idle' | 'loading' | 'running' | 'summary'

export type ColoringResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-highfreq-coloring-${q.difficulty}`,
    expression,
...
export function useColoringTest(difficulty: Ref<ColoringDifficulty | null>) {
  const phase = ref<ColoringPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<ColoringQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<ColoringResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : COLORING_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateColoringPaper(diff)
      if (generated.length < COLORING_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${coloringTopicLabel()} · ${coloringDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`op-highfreq-coloring-${difficulty.value}`, {
