...
export type RatioMultResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: RatioMultQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-skill-ratio-mult-${q.difficulty}`,
    expression,
...
export function useRatioMultTest(difficulty: Ref<RatioMultDifficulty | null>) {
  const phase = ref<RatioMultPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<RatioMultQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<RatioMultResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : RATIO_MULT_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateRatioMultPaper(diff)
      if (generated.length < RATIO_MULT_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${ratioMultTopicLabel()} · ${ratioMultDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`op-skill-ratio-mult-${difficulty.value}`, {
