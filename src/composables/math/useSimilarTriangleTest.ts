...
import {
  SIMILAR_TRIANGLE_QUESTION_COUNT,
  similarTriangleDifficultyLabel,
  similarTriangleTopicLabel,
  generateSimilarTrianglePaper,
...
export type SimilarTrianglePhase = 'idle' | 'loading' | 'running' | 'summary'

export type SimilarTriangleResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: SimilarTriangleQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-highfreq-similar-triangle-${q.difficulty}`,
    expression,
...
export function useSimilarTriangleTest(difficulty: Ref<SimilarTriangleDifficulty | null>) {
  const phase = ref<SimilarTrianglePhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<SimilarTriangleQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<SimilarTriangleResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : SIMILAR_TRIANGLE_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateSimilarTrianglePaper(diff)
      if (generated.length < SIMILAR_TRIANGLE_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${similarTriangleTopicLabel()} · ${similarTriangleDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`op-highfreq-similar-triangle-${difficulty.value}`, {
