...
import {
  YING_KUI_QUESTION_COUNT,
  generateYingKuiPaper,
  yingKuiDifficultyLabel,
  yingKuiTopicLabel,
  type YingKuiDifficulty,
  type YingKuiQuestion,
} from '@/utils/math/yingKuiPractice'
import { incrementPracticeCompletion } from '@/utils/app/practiceCompletionStats'

export type YingKuiPhase = 'idle' | 'loading' | 'running' | 'summary'

export type YingKuiResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: YingKuiQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-other-ying-kui-${q.difficulty}`,
    expression,
...
export function useYingKuiTest(difficulty: Ref<YingKuiDifficulty | null>) {
  const phase = ref<YingKuiPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<YingKuiQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<YingKuiResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : YING_KUI_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateYingKuiPaper(diff)
      if (generated.length < YING_KUI_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${yingKuiTopicLabel()} · ${yingKuiDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`op-other-ying-kui-${difficulty.value}`, {
