...
export type DatePhase = 'idle' | 'loading' | 'running' | 'summary'
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-other-date-${q.difficulty}`,
    expression,
...
export function useDateTest(difficulty: Ref<DateDifficulty | null>) {
  const phase = ref<DatePhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<DateQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<DateResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : DATE_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateDatePaper(diff)
      if (generated.length < DATE_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${dateTopicLabel()} · ${dateDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`op-other-date-${difficulty.value}`, {
