...
export type AgePhase = 'idle' | 'loading' | 'running' | 'summary'

export type AgeResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-other-age-${q.difficulty}`,
    expression,
...
export function useAgeTest(difficulty: Ref<AgeDifficulty | null>) {
  const phase = ref<AgePhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<AgeQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<AgeResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : AGE_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateAgePaper(diff)
      if (generated.length < AGE_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${ageTopicLabel()} · ${ageDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`op-other-age-${difficulty.value}`, {
