...
import {
  INCLUSION_EXCLUSION_QUESTION_COUNT,
  generateInclusionExclusionPaper,
...
export type InclusionExclusionPhase = 'idle' | 'loading' | 'running' | 'summary'

export type InclusionExclusionResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: InclusionExclusionQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-other-inclusion-exclusion-${q.difficulty}`,
    expression,
...
export function useInclusionExclusionTest(
  difficulty: Ref<InclusionExclusionDifficulty | null>,
) {
  const phase = ref<InclusionExclusionPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<InclusionExclusionQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<InclusionExclusionResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : INCLUSION_EXCLUSION_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateInclusionExclusionPaper(diff)
      if (generated.length < INCLUSION_EXCLUSION_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${inclusionExclusionTopicLabel()} · ${inclusionExclusionDifficultyLabel(q.difficulty)}`,
