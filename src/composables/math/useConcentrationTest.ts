...
import {
  CONCENTRATION_QUESTION_COUNT,
  concentrationDifficultyLabel,
  concentrationTopicLabel,
  generateConcentrationPaper,
...
export type ConcentrationPhase = 'idle' | 'loading' | 'running' | 'summary'

export type ConcentrationResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-highfreq-concentration-${q.difficulty}`,
    expression,
...
export function useConcentrationTest(difficulty: Ref<ConcentrationDifficulty | null>) {
  const phase = ref<ConcentrationPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<ConcentrationQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<ConcentrationResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : CONCENTRATION_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateConcentrationPaper(diff)
      if (generated.length < CONCENTRATION_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${concentrationTopicLabel()} · ${concentrationDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`op-highfreq-concentration-${difficulty.value}`, {
