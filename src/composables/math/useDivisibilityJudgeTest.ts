...
import {
  DIV_JUDGE_QUESTION_COUNT,
  divJudgeDifficultyLabel,
  divJudgeTopicLabel,
  generateDivisibilityJudgePaper,
...
export type DivJudgeResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: DivJudgeQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-skill-div-judge-${q.difficulty}`,
    expression,
...
export function useDivisibilityJudgeTest(difficulty: Ref<DivJudgeDifficulty | null>) {
  const phase = ref<DivJudgePhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<DivJudgeQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<DivJudgeResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : DIV_JUDGE_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateDivisibilityJudgePaper(diff)
      if (generated.length < DIV_JUDGE_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${divJudgeTopicLabel()} · ${divJudgeDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`op-skill-div-judge-${difficulty.value}`, {
