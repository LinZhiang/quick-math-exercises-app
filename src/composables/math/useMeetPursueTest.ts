...
import {
  MEET_PURSUE_QUESTION_COUNT,
  meetPursueDifficultyLabel,
  meetPursueTopicLabel,
  generateMeetPursuePaper,
...
export type MeetPursuePhase = 'idle' | 'loading' | 'running' | 'summary'

export type MeetPursueResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: MeetPursueQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-highfreq-meet-pursue-${q.difficulty}`,
    expression,
...
export function useMeetPursueTest(difficulty: Ref<MeetPursueDifficulty | null>) {
  const phase = ref<MeetPursuePhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<MeetPursueQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<MeetPursueResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : MEET_PURSUE_QUESTION_COUNT,
  )
  ...
  function generatePaper() {
    ...
    try {
      const generated = generateMeetPursuePaper(diff)
      if (generated.length < MEET_PURSUE_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${meetPursueTopicLabel()} · ${meetPursueDifficultyLabel(q.difficulty)}`,
