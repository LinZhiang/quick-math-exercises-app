...
export type FunctionGraphPhase = 'idle' | 'loading' | 'running' | 'summary'

export type FunctionGraphResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: FunctionGraphQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-other-function-graph-${q.difficulty}`,
    expression,
...
export function useFunctionGraphTest(difficulty: Ref<FunctionGraphDifficulty | null>) {
  const phase = ref<FunctionGraphPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<FunctionGraphQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<FunctionGraphResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : FUNCTION_GRAPH_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    phase.value = 'loading'
    loadingMessage.value = '豆包按函数图象图库出题（失败即用本地同图题）…'
    try {
      const generated = await requestFunctionGraphMcqs({
        count: FUNCTION_GRAPH_QUESTION_COUNT,
        difficulty: diff,
        avoidTerms: listRecentGeneratedTerms('op-other-function-graph'),
        onProgress: (msg) => {
          loadingMessage.value = msg
        },
      })
      if (generated.length < FUNCTION_GRAPH_QUESTION_COUNT) {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${functionGraphTopicLabel()} · ${functionGraphDifficultyLabel(q.difficulty)}`,
