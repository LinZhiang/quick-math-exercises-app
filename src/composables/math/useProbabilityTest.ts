...
export type ProbabilityPhase = 'idle' | 'loading' | 'running' | 'summary'

export type ProbabilityResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `op-highfreq-probability-${q.difficulty}`,
    expression,
...
export function useProbabilityTest(difficulty: Ref<ProbabilityDifficulty | null>) {
  const phase = ref<ProbabilityPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<ProbabilityQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<ProbabilityResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : PROBABILITY_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    if (diff === 'hard' && !isAiChatConfigured()) {
      ElMessage.warning(DEEPSEEK_NOT_CONFIGURED_HINT)
      return
    }
    phase.value = 'loading'
    loadingMessage.value =
      diff === 'hard' ? '豆包按几何概率题库出题（失败即用同型本地题）…' : '正在组卷…'
    try {
      let generated: ProbabilityQuestion[]
      if (diff === 'hard') {
        generated = await requestProbabilityHardMcqs({
          count: PROBABILITY_QUESTION_COUNT,
          avoidTerms: listRecentGeneratedTerms('op-highfreq-probability'),
          onProgress: (msg) => {
        ...
        generated = generateProbabilityPaper(diff)
      }
      if (generated.length < PROBABILITY_QUESTION_COUNT) {
      ...
      ElMessage.success(
        diff === 'hard'
          ? `已生成 ${questions.value.length} 道题（豆包）`
          : `已生成 ${questions.value.length} 道题`,
      )
    } catch (e) {
      phase.value = 'idle'
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${probabilityTopicLabel()} · ${probabilityDifficultyLabel(q.difficulty)}`,
