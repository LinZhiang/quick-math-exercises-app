...
import {
  ...
  dailyConclusionDifficultyLabel,
  dailyConclusionTopicLabel,
...
export type DailyConclusionPhase = 'idle' | 'loading' | 'running' | 'summary'

export type DailyConclusionResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: DailyConclusionQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `logic-reason-daily-conclusion-${q.difficulty}`,
    expression,
...
export function useDailyConclusionTest(difficulty: Ref<DailyConclusionDifficulty | null>) {
  const phase = ref<DailyConclusionPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<DailyConclusionQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<DailyConclusionResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : DAILY_CONCLUSION_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    try {
      const generated = await requestDailyConclusionMcqs({
        count: DAILY_CONCLUSION_QUESTION_COUNT,
        difficulty: diff,
        avoidTerms: listRecentGeneratedTerms('logic-reason-daily-conclusion'),
        onProgress: (msg) => {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${dailyConclusionTopicLabel()} · ${dailyConclusionDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`logic-reason-daily-conclusion-${difficulty.value}`, {
