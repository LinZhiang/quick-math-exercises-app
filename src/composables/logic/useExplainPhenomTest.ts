...
import {
  ...
  explainPhenomDifficultyLabel,
  explainPhenomTopicLabel,
...
export type ExplainPhenomPhase = 'idle' | 'loading' | 'running' | 'summary'

export type ExplainPhenomResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: ExplainPhenomQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `logic-reason-explain-${q.difficulty}`,
    expression,
...
export function useExplainPhenomTest(difficulty: Ref<ExplainPhenomDifficulty | null>) {
  const phase = ref<ExplainPhenomPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<ExplainPhenomQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<ExplainPhenomResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : EXPLAIN_PHENOM_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    try {
      const generated = await requestExplainPhenomMcqs({
        count: EXPLAIN_PHENOM_QUESTION_COUNT,
        difficulty: diff,
        avoidTerms: listRecentGeneratedTerms('logic-reason-explain'),
        onProgress: (msg) => {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${explainPhenomTopicLabel()} · ${explainPhenomDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`logic-reason-explain-${difficulty.value}`, {
