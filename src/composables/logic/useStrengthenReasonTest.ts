...
import {
  ...
  strengthenReasonDifficultyLabel,
  strengthenReasonTopicLabel,
...
export type StrengthenReasonPhase = 'idle' | 'loading' | 'running' | 'summary'

export type StrengthenReasonResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: StrengthenReasonQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `logic-reason-strengthen-${q.difficulty}`,
    expression,
...
export function useStrengthenReasonTest(difficulty: Ref<StrengthenReasonDifficulty | null>) {
  const phase = ref<StrengthenReasonPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<StrengthenReasonQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<StrengthenReasonResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : STRENGTHEN_REASON_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    try {
      const generated = await requestStrengthenReasonMcqs({
        count: STRENGTHEN_REASON_QUESTION_COUNT,
        difficulty: diff,
        avoidTerms: listRecentGeneratedTerms('logic-reason-strengthen'),
        onProgress: (msg) => {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${strengthenReasonTopicLabel()} · ${strengthenReasonDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`logic-reason-strengthen-${difficulty.value}`, {
