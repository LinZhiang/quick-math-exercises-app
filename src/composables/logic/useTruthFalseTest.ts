...
import {
  ...
  truthFalseDifficultyLabel,
  truthFalseTopicLabel,
...
export type TruthFalsePhase = 'idle' | 'loading' | 'running' | 'summary'

export type TruthFalseResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: TruthFalseQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `logic-reason-truth-false-${q.difficulty}`,
    expression,
...
export function useTruthFalseTest(difficulty: Ref<TruthFalseDifficulty | null>) {
  const phase = ref<TruthFalsePhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<TruthFalseQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<TruthFalseResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : TRUTH_FALSE_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    try {
      const generated = await requestTruthFalseMcqs({
        count: TRUTH_FALSE_QUESTION_COUNT,
        difficulty: diff,
        avoidTerms: listRecentGeneratedTerms('logic-reason-truth-false'),
        onProgress: (msg) => {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${truthFalseTopicLabel()} · ${truthFalseDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`logic-reason-truth-false-${difficulty.value}`, {
