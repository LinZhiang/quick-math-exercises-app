...
import {
  ...
  translationReasonDifficultyLabel,
  translationReasonTopicLabel,
...
export type TranslationReasonPhase = 'idle' | 'loading' | 'running' | 'summary'

export type TranslationReasonResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: TranslationReasonQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `logic-reason-translation-${q.difficulty}`,
    expression,
...
export function useTranslationReasonTest(difficulty: Ref<TranslationReasonDifficulty | null>) {
  const phase = ref<TranslationReasonPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<TranslationReasonQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<TranslationReasonResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : TRANSLATION_REASON_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    if (!diff) {
      ElMessage.warning('请先选择简单、普通或困难')
      return
    }
    ...
    try {
      const generated = await requestTranslationReasonMcqs({
        count: TRANSLATION_REASON_QUESTION_COUNT,
        difficulty: diff,
        avoidTerms: listRecentGeneratedTerms('logic-reason-translation'),
        onProgress: (msg) => {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${translationReasonTopicLabel()} · ${translationReasonDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`logic-reason-translation-${difficulty.value}`, {
