...
import {
  ...
  evalReasonDifficultyLabel,
  evalReasonTopicLabel,
...
export type EvalReasonPhase = 'idle' | 'loading' | 'running' | 'summary'

export type EvalReasonResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: EvalReasonQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `logic-reason-eval-reason-${q.difficulty}`,
    expression,
...
export function useEvalReasonTest(difficulty: Ref<EvalReasonDifficulty | null>) {
  const phase = ref<EvalReasonPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<EvalReasonQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<EvalReasonResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : EVAL_REASON_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    try {
      const generated = await requestEvalReasonMcqs({
        count: EVAL_REASON_QUESTION_COUNT,
        difficulty: diff,
        avoidTerms: listRecentGeneratedTerms('logic-reason-eval-reason'),
        onProgress: (msg) => {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${evalReasonTopicLabel()} · ${evalReasonDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`logic-reason-eval-reason-${difficulty.value}`, {
