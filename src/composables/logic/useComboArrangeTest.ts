...
export type ComboArrangePhase = 'idle' | 'loading' | 'running' | 'summary'

export type ComboArrangeResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: ComboArrangeQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `logic-reason-combo-arrange-${q.difficulty}`,
    expression,
...
export function useComboArrangeTest(difficulty: Ref<ComboArrangeDifficulty | null>) {
  const phase = ref<ComboArrangePhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<ComboArrangeQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<ComboArrangeResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : COMBO_ARRANGE_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    try {
      const generated = await requestComboArrangeMcqs({
        count: COMBO_ARRANGE_QUESTION_COUNT,
        difficulty: diff,
        avoidTerms: listRecentGeneratedTerms('logic-reason-combo-arrange'),
        onProgress: (msg) => {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${comboArrangeTopicLabel()} · ${comboArrangeDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`logic-reason-combo-arrange-${difficulty.value}`, {
