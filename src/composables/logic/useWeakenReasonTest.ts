...
import {
  ...
  weakenReasonDifficultyLabel,
  weakenReasonTopicLabel,
...
export type WeakenReasonPhase = 'idle' | 'loading' | 'running' | 'summary'

export type WeakenReasonResultRow = {
  unitIndex: number
  typeLabel: string
  title: string
  correct: boolean
  question: WeakenReasonQuestion
...
function saveWrong(pending: PendingWrong) {
  ...
  upsertMentalMathWrong({
    modeId: `logic-reason-weaken-${q.difficulty}`,
    expression,
...
export function useWeakenReasonTest(difficulty: Ref<WeakenReasonDifficulty | null>) {
  const phase = ref<WeakenReasonPhase>('idle')
  const loadingMessage = ref('')
  const questions = ref<WeakenReasonQuestion[]>([])
  const currentIndex = ref(0)
  const selectedIndex = ref<number | null>(null)
  const submitted = ref(false)
  const results = ref<WeakenReasonResultRow[]>([])
  ...
  const questionCount = computed(() =>
    questions.value.length > 0 ? questions.value.length : WEAKEN_REASON_QUESTION_COUNT,
  )
  ...
  async function generatePaper() {
    ...
    try {
      const generated = await requestWeakenReasonMcqs({
        count: WEAKEN_REASON_QUESTION_COUNT,
        difficulty: diff,
        avoidTerms: listRecentGeneratedTerms('logic-reason-weaken'),
        onProgress: (msg) => {
  ...
  function submitCurrent() {
    ...
    results.value.push({
      ...
      typeLabel: `${weakenReasonTopicLabel()} · ${weakenReasonDifficultyLabel(q.difficulty)}`,
  ...
  function nextQuestion() {
    ...
    if (currentIndex.value >= questions.value.length - 1) {
      finalizeElapsed()
      if (difficulty.value) {
        incrementPracticeCompletion(`logic-reason-weaken-${difficulty.value}`, {
