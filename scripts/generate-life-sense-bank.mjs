...
const ALL_KINDS = uniq(
  [...KINDS.easy, ...KINDS.normal, ...KINDS.hard]
    .map((x) => x[1])
    .concat(['菌类', '藻类', '豆类', '谷物', '蜂产品', '电器', '厨具', '家具', '文具', '蛋制品', '肉制品', '水产干货']),
)
const KIND_POOL = {
  easy: ALL_KINDS,
  normal: ALL_KINDS,
...
function addMaterialReverse(diff) {
  const rows = MATERIALS[diff]
  const shortOk = (p) =>
    p.length <= 4 && !/常见|主要|材质|主料|对应|俗称|填充|来自|基材|涂层|纤维|塑料|金属|橡胶|板/.test(p)
