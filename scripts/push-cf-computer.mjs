/**
 * 把本机 server/data/computer-basics 同步到 Cloudflare Pages（KV）
 * 用法：先部署带 Functions 的版本，并绑定 WENGU_KV，再执行 npm run sync:cf-computer
 */
...
const dataRoot = path.join(root, 'server', 'data', 'computer-basics')
...
async function main() {
  ...
  if (!password) {
    console.error('[sync:cf-computer] 请先在 server/.env 填写 WENGU_ADMIN_PASSWORD')
    process.exit(1)
  }
  const pack = loadPack()
  console.log(`[sync:cf-computer] 目标：${HOST}`)
  console.log(
    `[sync:cf-computer] 目录 ${pack.tree.length} 个根类，讲义 ${Object.keys(pack.items).length} 条，插图 ${Object.keys(pack.media).length} 张`,
  )
  ...
  try {
    ...
    console.error('[sync:cf-computer] 登录接口不是 JSON。请先部署本仓库的 Functions，再重试。')
    console.error(loginText.slice(0, 240))
    process.exit(1)
  }
  if (!loginRes.ok || !login.ok || !login.token) {
    console.error('[sync:cf-computer] 登录失败：', login.message || loginText.slice(0, 240))
    process.exit(1)
  }

  const importRes = await fetch(`${HOST}/api/computer-basics/import`, {
    method: 'POST',
    headers: {
  ...
  try {
    ...
    console.error('[sync:cf-computer] 导入接口不是 JSON（HTTP', importRes.status, '）')
    console.error(importText.slice(0, 240))
    process.exit(1)
  }
  if (!importRes.ok || !data.ok) {
    console.error('[sync:cf-computer] 导入失败：', data.message || importText.slice(0, 240))
...
main().catch((e) => {
  console.error('[sync:cf-computer]', e instanceof Error ? e.message : e)
  process.exit(1)
})
