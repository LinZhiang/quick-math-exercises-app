...
const STROKE = {
  ...
  忘: 7, 志: 7, 花: 7, 芽: 7, 码: 7, 矿: 8, 连: 7, 串: 7, 评: 7, 诉: 7, 识: 7, 词: 7,
...
/**
 * 封闭区域（用户约定 + 经典题）
 * 口=1 日=1 中=1 回=2 目=3 王=0；无五把吧=0→1→2→3；田=2（小学找规律计法）
 */
const ENCLOSE = {
  ...
  合: 1, 同: 1, 名: 1, 各: 1, 四: 1, 西: 1, 五: 1, 日: 1, 白: 1, 自: 1, 且: 1,
  回: 2, 田: 2, 吕: 2, 把: 2, 图: 2, 园: 2, 困: 2, 固: 2, 昌: 2, 串: 2, 国: 2,
  目: 3, 品: 3, 吧: 3, 晶: 3, 磊: 3, 鑫: 3,
  器: 4,
}

/** 不相连部分：开=1 勺=2 小=3 心=4 */
const PARTS = {
  ...
  勺: 2, 八: 2, 儿: 2, 丫: 2, 万: 2, 方: 2, 为: 2, 匀: 2, 勾: 2, 勿: 2, 匕: 2,
  小: 3, 川: 3, 彡: 3,
  心: 4, 必: 4,
}
...
/** 左右对称（含「贵」按教学约定）；上下对称含「巨」 */
const SYM_LR = [
  '林', '羽', '朋', '双', '非', '木', '米', '十', '口', '回', '田', '中', '一', '二', '三', '八',
  '小', '山', '井', '开', '丰', '王', '主', '玉', '日', '目', '且', '自', '火', '大', '天', '夫', '贵',
]
const SYM_UD = ['巨', '二', '三', '吕', '昌', '圭', '炎', '目', '一', '亘', '亚', '出', '兆', '芈']
...
function tipFor(chars, correct) {
  ...
  if (correct === '左右对称') return '四字均为左右轴对称'
  if (correct === '上下对称') return '四字均为上下轴对称'
...
add(['无', '五', '把', '吧'], '封闭区域个数累加1', '封闭区域 0→1→2→3（经典例）', { priority: true })
...
add(['林', '羽', '朋', '双'], '左右对称', '林/羽/朋/双均为左右对称', { priority: true })
add(['贵', '林', '朋', '双'], '左右对称', '贵按教学约定左右对称；贵/林/朋/双', { priority: true })
add(['巨', '吕', '圭', '炎'], '上下对称', '巨/吕/圭/炎均为上下对称（巨=上下对称）', { priority: true })
...

{
  ...
  for (const q of sampleComb4(SYM_LR, 20, 3)) add(q, '左右对称', null, { extraDist: extra })
  for (const q of windowQuartets(SYM_UD, 1)) add(q, '上下对称', null, { extraDist: extra })
  for (const q of sampleComb4(SYM_UD, 20, 5)) add(q, '上下对称', null, { extraDist: extra })
}
