...
/**
 * 笔画交叉数（仅收录已核验字；计「笔画穿过形成的交叉点」）
 * 锚点：才=1（一横穿竖）；干=2（两横各穿竖）；丰=3；井=4（两横×两竖）。
 * 人/八/小等相接或分立、无「穿过」的，计 0。
 */
const CROSS = {
  一: 0, 二: 0, 三: 0, 川: 0, 八: 0, 小: 0, 人: 0, 入: 0, 儿: 0, 了: 0, 丫: 0, 彡: 0, 乙: 0,
  十: 1, 七: 1, 九: 1, 力: 1, 刀: 1, 丈: 1, 才: 1, 支: 1, 友: 1, 大: 1, 又: 1,
  干: 2, 于: 2, 午: 2, 牛: 2, 开: 2, 屯: 2, 并: 2, 关: 2, 升: 2,
  丰: 3, 韦: 3, 未: 3, 末: 3, 朱: 3, 束: 3, 连: 3, 来: 3,
  井: 4, 米: 4, 夹: 4, 爽: 4,
}
{
  if (CROSS['才'] !== 1) throw new Error('CROSS 才 必须为 1')
  if (CROSS['干'] !== 2) throw new Error('CROSS 干 必须为 2（两横穿竖）')
  if (CROSS['丰'] !== 3) throw new Error('CROSS 丰 必须为 3')
  if (CROSS['井'] !== 4) throw new Error('CROSS 井 必须为 4')
  if (CROSS['十'] !== 1 || CROSS['二'] !== 0) throw new Error('CROSS 二/十 锚点错误')
}
...
const ENCLOSE = {
  ...
  五: 0, 于: 0, 升: 0, 并: 0, 关: 0, 屯: 0, 韦: 0,
  口: 1, 中: 1, 古: 1, 右: 1, 石: 1, 可: 1, 后: 1, 甲: 1, 申: 1, 电: 1, 由: 1,
  合: 1, 同: 1, 名: 1, 各: 1, 西: 1, 日: 1, 白: 1, 自: 1,
  把: 1, 图: 1, 园: 1, 困: 1, 国: 1,
  且: 2, 四: 2, 吧: 2, 回: 2, 田: 2, 吕: 2, 固: 2, 昌: 2, 串: 2,
  目: 3, 品: 3, 晶: 3, 磊: 3, 鑫: 3,
  器: 4,
}
{
  if (ENCLOSE['日'] !== 1 || ENCLOSE['昌'] !== 2 || ENCLOSE['晶'] !== 3) {
    throw new Error('ENCLOSE 日/昌/晶 须为 1/2/3（木日昌晶累加）')
  }
  if (ENCLOSE['且'] !== 2 || ENCLOSE['四'] !== 2) throw new Error('ENCLOSE 且/四 须为 2')
  if (ENCLOSE['才'] !== 0 || ENCLOSE['干'] !== 0) throw new Error('ENCLOSE 才/干 须为 0')
  if (ENCLOSE['目'] !== 3 || ENCLOSE['田'] !== 2) throw new Error('ENCLOSE 目/田 锚点错误')
}
 ...
 * 硬规则：
 * - 两表互斥。
 * - 「小/八」禁止进入左右对称：点画分立，不作左右对称考点（勿与「古/山/大」混用）。
 * - 「且/凹/凸/皿」是左右对称，绝不是上下对称。
 * - 「甲/由」仅左右对称，绝不是上下对称。
 * - 「禾/永」有撇捺，非轴对称，不进表。
 * - 仅上下、非左右的锚点字：巨/臣/叵。上下对称题必须至少含其中一字。
 * - 「中」只进上下对称表（双轴字按上下对称考点收录，不进左右对称表）。
 */
const SYM_LR = [
  '古', '山', '大', '木', '非', '米', '天', '夫', '火', '开', '丰', '井',
  '未', '末', '来', '本', '夹', '羊', '半', '平', '辛',
  '林', '羽', '朋', '双',
  '且', '凹', '凸', '皿', '甲', '由',
]
/** 禁止当作左右对称的字（易误收） */
const SYM_LR_FORBIDDEN = ['小', '八', '中', '日', '目', '口', '田', '回', '申']
/** 仅上下对称、非左右对称（侧向开口） */
...

{
  ...
  for (const ch of SYM_LR_FORBIDDEN) {
    if (lr.has(ch)) throw new Error(`SYM_LR 禁止收录：${ch}`)
  }
  for (const ch of ['古', '山', '大']) {
    if (!lr.has(ch)) throw new Error(`SYM_LR 必须含锚点：${ch}`)
  }
}
...
function validateSeed(chars, correct) {
  ...
  if (correct === '笔画数相等') {
    ...
    if (c.some((ch) => SYM_LR_FORBIDDEN.includes(ch))) return '含禁止左右对称字（如小/八）'
    if (c.every((ch) => SYM_UD.includes(ch))) return '同时满足上下对称'
    for (const [lab, list] of Object.entries(STRUCT)) {
      if (c.every((ch) => list.includes(ch))) return `同时满足${lab}`
    }
...
function tipFor(chars, correct) {
  ...
  if (correct === '左右对称') return '四字均为左右对称（沿竖轴对折重合，如古/山/大；不含小/八）'
